...
// Encoding: sqrtPriceX64 = sqrt(A_atomic/B_atomic) * 2^64, where A_atomic/B_atomic = (A_whole/B_whole) * 10^(decB-decA)
...
describe('raydium clmm sqrt decode', () => {
  it('decodes A-per-1-B from sqrtPriceX64 and decimals', async () => {
    ...
    // Encode: A_atomic/B_atomic = target * 10^(decB-decA) = 200 * 10^3 = 200000
    // sqrt = sqrt(200000) * 2^64
    const atomicPrice = target * Math.pow(10, decB - decA);
