...
// Raydium CLMM decode sanity: A-per-1-B = 10^(decB - decA) / (ratio^2), ratio = sqrt / 2^64
// Same formula as Orca CLMM
describe('raydium clmm sqrt decode', () => {
  it('decodes A-per-1-B from sqrtPriceX64 and decimals', async () => {
    ...
    // Encode: ratio = sqrt(10^(decB - decA) / target), then sqrt = ratio * 2^64
