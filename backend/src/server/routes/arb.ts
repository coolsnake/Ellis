...
} from "../../workers/arbBuild.types.js";
...
} from "../../execution/slippage/index.js";
...
export function createArbRouter(io: SocketIOServer): Router {
  ...
  const ARB_BUILD_WORKER_DISABLED =
    String(
      env.ARB_BUILD_WORKER_DISABLED ?? env.ARB_BUILD_TX_DISABLED ?? ""
    ).toLowerCase() === "true";
  const ARB_BUILD_WORKER_MAX_QUEUE = Math.max(
    1,
    Number(env.ARB_BUILD_WORKER_MAX_QUEUE ?? 2)
  );
  const ARB_BUILD_WORKER_TIMEOUT_MS = Math.max(
    2000,
    Number(env.ARB_BUILD_WORKER_TIMEOUT_MS ?? 8000)
  );
  const ARB_BUILD_WORKER_IDLE_MS = Math.max(
    0,
    Number(env.ARB_BUILD_WORKER_IDLE_MS ?? 30_000)
  );
  const ARB_BUILD_WORKER_CONCURRENCY = Math.max(
    1,
    Number(env.ARB_BUILD_WORKER_CONCURRENCY ?? 1)
  );

  let arbBuildWorkerClient: WorkerClient<
    ArbBuildRequest,
    ArbBuildResult
  > | null = null;
  ...
  async function assertPlanMatchesGraph(
    path: string[],
    hops: Array<{ poolId: string }>
  ): Promise<void> {
    try {
      ...
      for (let i = 0; i < hops.length; i += 1) {
        ...
        if (!e) {
          try {
            logger.warn("arb.graph_plan.missing_edge", {
              hop: i,
              pid,
              src,
              dst,
        ...
        if (
          String((e as any).source) !== src ||
          String((e as any).target) !== dst
        ) {
          try {
            logger.warn("arb.graph_plan.mismatch", {
              hop: i,
              pid,
              src,
              dst,
              edgeSrc: (e as any).source,
              edgeDst: (e as any).target,
              cat: "arb",
            });
          } catch {}
        }
  ...
  api.get("/arb/config", async (_req, res) => {
    try {
      ...
      let local: any = {};
      try {
        local = await readJson("backend/config/arbConfig.json", {} as any);
      } catch {}
      ...
      logger.warn("arb.config.get.error", {
        cat: "arb",
        error: String(e?.message || e),
      });
      try {
        const local = await readJson(
          "backend/config/arbConfig.json",
          {} as any
        );
  ...
  api.get("/arb/graph/sync-status", async (_req, res) => {
    try {
      const { getGraphVersion } = await import("../graph.js");
      const { isArbStreamEnabled, getCachedArbVersion } = await import(
        "../realtime.js"
      );
      ...
      const status = {
        ...
        sync_status:
          streamEnabled && versionLag === 0 && timeLag < 5000
            ? "synced"
            : streamEnabled && versionLag <= 2 && timeLag < 30000
            ? "lagging"
            : streamEnabled
            ? "out_of_sync"
            : "disabled",
  ...
  api.get("/arb/opportunities", async (_req, res) => {
    try {
      ...
      res
        .status(r?.status || 503)
        .json(r ? await r.json().catch(() => ({})) : { ok: false });
    } catch {
      res.status(503).json({ ok: false });
    }
  });
  ...
  api.get("/arb/calibration/stats", async (_req, res) => {
    res.status(410).json({
      ok: false,
      error: "Calibration removed - sizing now handled by arb-rs",
    });
  });
  ...
  api.post("/arb/calibration/reset", async (_req, res) => {
    try {
      res.status(200).json({
        ok: true,
        message: "Calibration removed - sizing now handled by arb-rs",
      });
    } catch (e: any) {
      logger.warn("arb.calibration.reset.error", {
        cat: "arb",
        error: toErrString(e),
      });
  ...
  api.get("/arb/metrics/json", async (_req, res) => {
    try {
      ...
      try {
        const { getPoolsMetrics, getPoolCacheAges } = await import(
          "../pools.js"
        );
        ...
        const exec = {
          build_ms: {
            p50: pct(execStats.buildMs, 50),
            p95: pct(execStats.buildMs, 95),
          },
          preflight_ms: {
            p50: pct(execStats.preflightMs, 50),
            p95: pct(execStats.preflightMs, 95),
          },
          send_ms: {
            p50: pct(execStats.sendMs, 50),
            p95: pct(execStats.sendMs, 95),
          },
          counts: {
            preflight_ok: execStats.preflightOk,
            preflight_err: execStats.preflightErr,
            send_ok: execStats.sendOk,
            send_err: execStats.sendErr,
          },
        };
        ...
        try {
          const { dexAltManager } = await import(
            "../../execution/utils/altManager.js"
          );
        ...
        try {
          ...
          const countDex = (dex: string) => {
            ...
            return {
              edges: (snap?.edges || []).filter((e: any) => e.dex === dex)
                .length,
              pools: pools.size,
            };
          };
  ...
  api.post("/arb/alts/extend", async (req, res) => {
    try {
      ...
      if (accounts.length === 0) {
        return res
          .status(400)
          .json({ error: "accounts array is required and cannot be empty" });
      }
  ...
  api.post("/arb/alts/create", async (req, res) => {
    try {
      ...
      const result = await dexAltManager.createAndExtendAlt(
        category,
        accounts,
        seed
      );
  ...
  api.get("/arb/alts/collect-accounts", async (req, res) => {
    try {
      const category = String(req.query.category || "common") as
        | "common"
        | "pools"
        | "tokens"
        | "clmm"
      ...
      const accounts = await dexAltManager.collectAccountsForCategory(
        category,
        {
  ...
  api.post("/arb/alts/extend-with-category", async (req, res) => {
    try {
      ...
      const accountCategory = String(body.accountCategory || "common") as
        | "common"
        | "pools"
        | "tokens"
        | "clmm"
      ...
      if (accounts.length === 0) {
        return res
          .status(400)
          .json({ error: "No accounts collected for the specified category" });
      }
  ...
  api.get("/arb/alts/pools-by-dex", async (req, res) => {
    try {
      const dex = String(req.query.dex || "raydium") as
        ...
        | "meteora-damm-v2"
        | "pumpswap";
      const poolType = String(req.query.poolType || "both") as
        | "amm"
        | "clmm"
        | "dlmm"
        | "cpmm"
      ...
      const maxPools = Math.min(
        100,
        Math.max(1, Number(req.query.maxPools) || 30)
      );
      ...
      const dexMatchFn = (edgeDex: string, edgePoolKind: string): boolean => {
        ...
        switch (dex) {
          ...
          case "meteora-balanced":
            return (
              normalizedEdgeDex === "meteora_balanced" ||
              normalizedEdgeDex === "meteora-balanced"
            );
          case "meteora-damm-v1":
            // Match Meteora Dynamic AMM v1 pools
            return (
              normalizedEdgeDex === "meteora_damm_v1" ||
              normalizedEdgeDex === "meteora-damm-v1" ||
              normalizedEdgeDex === "meteorabalanced_v1" ||
              normalizedEdgeDex === "meteora_balanced_v1"
            );
          case "meteora-damm-v2":
            // Match Meteora CP-AMM v2 pools
            return (
              normalizedEdgeDex === "meteora_damm_v2" ||
              normalizedEdgeDex === "meteora-damm-v2" ||
              normalizedEdgeDex === "meteorabalanced_v2" ||
              normalizedEdgeDex === "meteora_balanced_v2"
            );
          case "pumpswap":
      ...
      forwardEdgesOnly.sort((a, b) => {
        const getLiquidity = (edge: any): number => {
          ...
          if (edge.liquidity_display && edge.liquidity_display > 0)
            return edge.liquidity_display;
          if (edge.pool_liquidity_raw && edge.pool_liquidity_raw > 0)
            return edge.pool_liquidity_raw;
      ...
      for (const edge of forwardEdgesOnly) {
        ...
        const tvl =
          edge.tvl_usd ||
          edge.liquidity_display ||
          edge.pool_liquidity_raw ||
          edge.liquidity ||
          0;
  ...
  api.post("/arb/alts/create-dex-alt", async (req, res) => {
    try {
      ...
      const dex = String(body.dex || "raydium") as
        ...
        | "meteora-damm-v2"
        | "pumpswap";
      const poolType = String(body.poolType || "both") as
        | "amm"
        | "clmm"
        | "dlmm"
        | "cpmm"
      ...
      const category = String(
        body.category || `${dex}-${poolType === "both" ? "all" : poolType}`
      );
      ...
      const accounts = await dexAltManager.collectDexPoolAccounts(
        dex,
        poolType,
        maxPools
      );
      ...
      if (accounts.length === 0) {
        return res.status(400).json({
          error: "No accounts collected for the specified DEX and pool type",
        });
      }
  ...
  api.post("/arb/alts/refresh-dex-alt", async (req, res) => {
    try {
      ...
      if (accounts.length === 0) {
        return res
          .status(400)
          .json({ error: "No accounts collected for refresh" });
      }
  ...
  api.post("/arb/alts/deactivate", async (req, res) => {
    try {
      ...
      res.json({
        ...
        ...result,
        message:
          "ALT deactivated. Wait ~5 minutes (513 slots) before closing to recover rent.",
      });
    } catch (e: any) {
      res.status(500).json({ error: String(e?.message || e) });
    }
  ...
  api.post("/arb/alts/close", async (req, res) => {
    try {
      ...
      res.json({
        ...
        message: `ALT closed. Recovered ${(result.rentRecovered / 1e9).toFixed(
          6
        )} SOL`,
      });
    } catch (e: any) {
  ...
  api.post("/arb/alts/reinitialize", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message:
          "ALT manager re-initialized successfully. Deleted ALTs have been cleaned up.",
      });
    } catch (e: any) {
      res.status(500).json({ error: String(e?.message || e) });
    }
  ...
  api.post("/arb/clmm/refresh", async (req: Request, res: Response) => {
    try {
      const poolId = String(
        (req.body && (req.body as any).poolId) ||
          (req.query && (req.query as any).poolId) ||
          ""
      );
  ...
  api.get("/arb/graph/current", async (req, res) => {
    try {
      ...
      if (
        !snap ||
        !Array.isArray((snap as any).edges) ||
        (snap as any).edges.length === 0
      ) {
      ...
      try {
        logger.error("arb.graph.current error", {
          error: String(e?.message || e),
        });
      } catch {}
  ...
  api.post("/arb/start", async (req, res) => {
    try {
      ...
      // Enable streaming if we sent a graph — even if arb-rs timed out or returned an error.
      // The snapshot was already sent over the wire; arb-rs may have received it.
      // Streaming ensures subsequent diffs keep arb-rs in sync.
      if (wantEnable) {
        const shouldEnable = includeGraph || (r !== null && j?.ok);
        if (shouldEnable) {
      ...
      // Return 200 if we enabled streaming (even if arb-rs timed out) so frontend doesn't show error
      const responseStatus =
        wantEnable && includeGraph && r === null ? 200 : status;
      const responseBody =
        wantEnable && includeGraph && r === null
          ? {
              ok: true,
              timedOut: true,
              message:
                "Snapshot sent; arb-rs response timed out but streaming enabled",
            }
          : j;
  ...
  api.post("/arb/simulate", async (req, res) => {
    try {
      try {
        emit("log", {
          level: "info",
          message: "pretrade:arb simulate start",
          timestamp: new Date().toISOString(),
          context: { cat: "arb", code: "PRETRADE.SIM.START" },
        });
      } catch {}
      const { resolveDirectPlan } = await import(
        "../../execution/resolver/index.js"
      );
      ...
      const basePlan =
        input && (input as any).plan && Array.isArray((input as any).plan?.hops)
          ? (input as any).plan
          : undefined;
      ...
      if (basePlan) {
        for (
          let i = 0;
          i < plan.hops.length && i < basePlan.hops.length;
          i += 1
        ) {
          ...
          if (src.minOutRaw !== undefined && src.minOutRaw !== null) {
            try {
              plan.hops[i].minOutRaw = BigInt(String(src.minOutRaw));
            } catch {}
          }
        }
      }
      ...
      const oppKey = (() => {
        try {
          const fams = Array.from(
            new Set((plan.hops || []).map((h: any) => String(h?.dex || "")))
          )
            .filter(Boolean)
            .sort();
          return `${(plan.path || []).join("->")}|${fams.join(",")}`;
        } catch {
      ...
      try {
        pushBounded(execStats.buildMs, Date.now() - tBuild0);
      } catch {}
      try {
        emit("log", {
          level: "info",
          message: "pretrade:arb tx built",
          timestamp: new Date().toISOString(),
          context: {
            cat: "tx",
            code: "PRETRADE.TX.BUILT",
            mode: (execCfg as any)?.mode,
          },
        });
      } catch {}
      try {
  ...
  api.post("/arb/simulate-send", async (req, res) => {
    try {
      ...
      try {
        await assertPlanMatchesGraph(plan.path, plan.hops as any);
      } catch {}
      const { getGraphVersion } = await import("../graph.js");
      const graph = getGraphVersion();
      const oppKey = (() => {
      ...
      const earlyIntent = {
        ...
        hops: (plan.hops || []).map((h: any) => ({
          ...
          inputDecimals: h.inputDecimals,
          outputDecimals: h.outputDecimals,
          inputTokenProgram: h.inputTokenProgram,
          outputTokenProgram: h.outputTokenProgram,
          amountInRaw:
            typeof h.amountInRaw === "bigint"
              ? h.amountInRaw.toString()
              : String(h.amountInRaw || 0),
          minOutRaw:
            typeof h.minOutRaw === "bigint"
              ? h.minOutRaw.toString()
              : String(h.minOutRaw || 0),
          userSourceAta: h.userSourceAta,
          userDestAta: h.userDestAta,
          ...(h.vaultA ? { vaultA: h.vaultA } : {}),
          ...(h.vaultB ? { vaultB: h.vaultB } : {}),
          ...(h.tickSpacing ? { tickSpacing: h.tickSpacing } : {}),
          ...(h.sqrtPriceLimitX64
            ? { sqrtPriceLimitX64: String(h.sqrtPriceLimitX64) }
            : {}),
        ...
        poolCache: (plan.hops || []).map((h: any) => {
          ...
          const hot = hotRaw
            ? {
                sqrtPriceX64:
                  typeof hotRaw.sqrtPriceX64 === "bigint"
                    ? hotRaw.sqrtPriceX64.toString()
                    : hotRaw.sqrtPriceX64,
      ...
      const id = Math.random().toString(36).slice(2, 10);
      try {
        logger.info("tx.intents", {
          cat: "tx",
          ctx: { id, intent: earlyIntent } as any,
        });
      } catch {}
      try {
        emit("log", {
          level: "info",
          message: "tx.intents",
          context: { cat: "tx", id, intent: earlyIntent },
        });
      } catch {}
      ...
      const ixs = built.instructions.map((ix) => {
        try {
          ...
          return {
            pid: String((ix as any)?.programId || ""),
            dataLen: 0,
            accounts: [],
          };
        }
      ...
      try {
        ...
        for (const pid of progs) {
          if (
            pid === String(v?.raydium?.ammV4Program) ||
            pid === String(v?.raydium?.ammV5Program) ||
            pid === String(v?.raydium?.clmmProgram)
          )
            seen.add("raydium");
        ...
        if (JSON.stringify(planned) !== JSON.stringify(seenArr)) {
          try {
            logger.warn("tx.dex_mismatch.preflight", {
              planned,
              seen: seenArr,
              cat: "tx",
            });
      ...
      const altAddresses =
        built.lookupTableAddresses || execCfg.lookupTableAddresses || [];
      ...
      try {
        const dexes = Array.from(
          new Set(
            (plan.hops || []).map((h: any) =>
              String(h?.dex || "").toLowerCase()
            )
          )
        );
        const txLogs = getTxRelatedLogs(
          id,
          Date.now() - 30000,
          Date.now(),
          200
      ...
      try {
        pushBounded(execStats.preflightMs, Date.now() - tPre0);
      } catch {}

      // reuse early id
      try {
        logger.info("tx.intents", { cat: "tx", ctx: { id, intent } as any });
      } catch {}
      try {
        logger.info("tx.ixs", {
          cat: "tx",
          ctx: { id, ixCount: built.ixCount, items: ixs } as any,
        });
      } catch {}
      try {
        emit("log", {
      ...
      try {
        emit("log", {
          level: "info",
          message: "tx.ixs",
          context: { cat: "tx", id, ixCount: built.ixCount, items: ixs },
        });
      } catch {}
        ...
        if ((sim as any)?.err) {
          ...
          logger.info("tx.preflight.err", {
            ...
            ctx: {
              ...
              txSizeBytes: built.sizeBytes,
              mode: (execCfg as any)?.mode,
              logCount: Array.isArray((sim as any)?.logs)
                ? (sim as any).logs.length
                : 0,
              error: errStr,
            } as any,
          });
        } else {
          logger.info("tx.preflight.ok", {
            cat: "tx",
            code: LogCode.TX_PREFLIGHT_OK,
            ctx: {
              id,
              ixCount: built.ixCount,
              txSizeBytes: built.sizeBytes,
      ...
      try {
        emit("log", {
          level: "info",
          message: "pretrade:arb simulate result",
          timestamp: new Date().toISOString(),
          context: {
            cat: "arb",
            code: "PRETRADE.SIM.END",
            ...((sim as any)?.err ? { err: String((sim as any).err) } : {}),
          },
        });
      } catch {}
      try {
  ...
  api.post("/arb/jupiter/roundtrip", async (req, res) => {
    try {
      const body = req.body || {};
      const sizeSol = Number.isFinite(body?.sizeSol)
        ? Number(body.sizeSol)
        : 0.01;
      const slippageBps = Number.isFinite(body?.slippageBps)
        ...
        : 50;
      try {
        logger.info("jupiter.trade.api.roundtrip", {
          cat: "jupiter",
          sizeSol,
          slippageBps,
        });
      } catch {}
      const { executeRoundtripWithJupiter } = await import(
        "../../jupiter/arbExecutor.js"
      );
      const r = await executeRoundtripWithJupiter({ sizeSol, slippageBps });
      res.json({ signature: r.signature });
    } catch (e: any) {
      try {
        logger.info("jupiter.trade.api.roundtrip.err", {
          cat: "jupiter",
          error: String(e?.message || e),
        });
      } catch {}
  ...
  api.post("/arb/jupiter/execute", async (req, res) => {
    try {
      ...
      const sizeTokens = Number.isFinite(body.size)
        ? Number(body.size)
        : undefined;
      const sizeUsd = Number.isFinite(body.sizeUsd)
        ? Number(body.sizeUsd)
        : undefined;
      const slippageBps = Number.isFinite(body.slippageBps)
        ? Number(body.slippageBps)
        : undefined;
      const hopDexes: string[] | undefined = Array.isArray(body.hopDexes)
        ? body.hopDexes
        : undefined;
      const hopRatesUi: number[] | undefined = Array.isArray(body.hopRates)
        ? body.hopRates
        : undefined;
      const hopMinOutsAtoms: number[] | undefined = Array.isArray(
        body.hopMinOutsAtoms
      )
        ? body.hopMinOutsAtoms
        : undefined;
      ...
      if (Number.isFinite(sizeTokens)) {
        try {
          ...
          sizeAtoms = Math.max(
            0,
            Math.floor(Number(sizeTokens) * Math.pow(10, dec0))
          );
        } catch {}
      }
      try {
        logger.info("jupiter.trade.api.execute", {
          cat: "jupiter",
          pathLen: path.length,
          size: sizeTokens,
          sizeUsd,
          slippageBps,
          strictMinOut,
          hopDexesLen: Array.isArray(hopDexes) ? hopDexes.length : 0,
        });
      } catch {}
      const { executePlanWithJupiterStrict } = await import(
        "../../jupiter/arbExecutor.js"
      );
      const r = await executePlanWithJupiterStrict({
        plan: { path },
        sizeAtoms,
        sizeUsd,
        slippageBps,
      ...
      try {
        logger.info("jupiter.trade.api.execute.err", {
          cat: "jupiter",
          error: String(e?.message || e),
        });
      } catch {}
  ...
  api.post("/arb/jupiter/aggregate-execute", async (req, res) => {
    try {
      ...
      if (Number.isFinite(body.size)) {
        try {
          const { resolveMint } = await import("../../utils/tokens.js");
          const dec0 = (await resolveMint(inputMint)).decimals ?? 6;
          sizeAtoms = Math.max(
            0,
            Math.floor(Number(body.size) * Math.pow(10, dec0))
          );
        } catch {}
      }
      const sizeUsd = Number.isFinite(body.sizeUsd)
      ...
      const hopDexes: string[] = Array.isArray(body.hopDexes)
        ? body.hopDexes
        : [];
      const families = Array.from(
        new Set(
          hopDexes.map((x: string) => {
      ...
      const { executeAggregateWithJupiter } = await import(
        "../../jupiter/arbExecutor.js"
      );
      const r = await executeAggregateWithJupiter({
        inputMint,
        outputMint,
        sizeAtoms,
        sizeUsd,
        slippageBps,
        dexWhitelist: families,
  ...
  api.post("/arb/execute", async (req, res) => {
    ...
    try {
      try {
        emit("log", {
          level: "info",
          message: "pretrade:arb execute start",
          timestamp: new Date().toISOString(),
          context: { cat: "arb", code: "PRETRADE.EXEC.START" },
        });
      } catch {}
      try {
        logger.info("pretrade:arb execute start", {
          cat: "arb",
          code: LogCode.API_REQUEST,
        });
      } catch {}
      const { resolveDirectPlan } = await import(
        "../../execution/resolver/index.js"
      ...
      const { ResolveDirectSchema } = await import("../routes/schemas.js");
      const { assembleAndSend, assembleAndSimulate } = await import(
        "../../execution/sender.js"
      );
      const { addTxRecord } = await import("../txHistory.js");
      const { loadExecConfig } = await import("../execConfigStore.js");
      ...
      try {
        ...
        try {
          const { writeTxFullDump } = await import("../../utils/txTrace.js");
          const { getTxRelatedLogs } = await import(
            "../../utils/sessionLogs.js"
          );
          const dexes = Array.from(
            new Set((plan.hops || []).map((h: any) => h.dex))
          );
          const txLogs = getTxRelatedLogs(
            id,
            Date.now() - 30000,
      ...
      if (mode === "jupiter" && !forceDirect) {
        try {
          ...
          const hopDexes: string[] = Array.isArray((input as any)?.dexes)
            ? (input as any).dexes
            : Array.isArray((plan as any)?.hops)
            ? (plan as any).hops.map((h: any) => String(h?.dex || ""))
            : [];
          ...
          try {
            if (Number.isFinite((input as any)?.size as any)) {
              ...
              sizeAtoms = Math.max(
                0,
                Math.floor(Number((input as any).size) * Math.pow(10, dec0))
              );
            }
          } catch {}
          const signature = (
            await executePlanWithJupiterStrict({
              plan,
              sizeAtoms,
              sizeUsd: Number.isFinite((input as any)?.sizeUsd as any)
                ? Number((input as any).sizeUsd)
                : undefined,
              slippageBps: Number.isFinite((input as any)?.slippageBps as any)
                ? Number((input as any).slippageBps)
                : undefined,
              hopDexes,
              strictMinOut: true,
            })
          ...
          return res
            .status(400)
            .json({ id, mode, error: "jupiter_exec_failed" });
        }
      }
      ...
      try {
        ...
        try {
          logger.info("wallet.ready", {
            cat: "wallet",
            ctx: { address: kp.publicKey.toBase58() } as any,
          });
        } catch {}
      } catch (e: any) {
        try {
          logger.info("wallet.missing", {
            cat: "wallet",
            ctx: { error: String(e?.message || e) } as any,
          });
        } catch {}
      ...
      if (maxBytes > 0 && built.sizeBytes > maxBytes) {
        try {
          logger.info("tx.size.warn", {
            cat: "tx",
            ctx: {
              ixCount: built.ixCount,
              txSizeBytes: built.sizeBytes,
              maxTxSizeBytes: maxBytes,
            } as any,
          });
        } catch {}
      }
      ...
      const skipPresendSim =
        (CONFIG as any)?.execution?.skipPresendSimulation === true;
        ...
        await logTxTrace("preflight", {
          ...
          exec: {
            ...
            computeUnitPriceMicroLamports:
              execCfg.computeUnitPriceMicroLamports,
            lookupTableAddresses: execCfg.lookupTableAddresses,
          },
          intent,
          ixs,
        ...
        if ((sim as any)?.err) {
          ...
          try {
            await addTxRecord({
              ...
              signature: null,
              status: "sim_err",
              error: String((sim as any)?.err),
              logFile: simLogFile,
            });
            try {
              emit("tx:history.updated", { id, status: "sim_err" });
            } catch {}
          } catch {}
          return res.status(400).json({
            id,
      ...
      try {
        ...
        const { quoteHopOut } = await import(
          "../../execution/resolver/quotes.js"
        );
        const slippageCheckBps = Number(execCfg.slippageBpsDefault || 50);
        const maxSlippageBps = Number(
          (CONFIG as any)?.system?.maxAllowedSlippageBps || 200
        ); // 2% max slippage
        ...
        if (originalAmountOut > 0n && expectedOut > 0n) {
          const actualSlippageBps = Number(
            ((expectedOut - originalAmountOut) * 10000n) / expectedOut
          );
          if (actualSlippageBps > maxSlippageBps) {
            try {
              logger.warn("tx.slippage.exceeded", {
                cat: "tx",
                ctx: {
                  id,
                  expectedSlippage: slippageCheckBps,
                  actualSlippageBps,
        ...
        try {
          logger.warn("tx.slippage.check.failed", {
            cat: "tx",
            ctx: { id, error: String(e?.message || e) } as any,
          });
        } catch {}
      ...
      try {
        ...
        try {
          pushBounded(execStats.sendMs, Date.now() - tSend0);
          execStats.sendOk += 1;
        } catch {}
        ...
        try {
          logger.info("tx.send.ok", {
            cat: "tx",
            code: LogCode.TX_SEND_OK,
            ctx: {
              id,
              signature,
              ixCount: built.ixCount,
        ...
        try {
          emit("log", {
            level: "info",
            message: signature ? "arb:send ok" : "arb:send err",
            timestamp: new Date().toISOString(),
            context: {
              cat: "arb",
              mode: forceDirect ? "direct(force)" : mode,
        ...
        try {
          try {
            logger.info("tx.send.err", {
              cat: "tx",
              code: LogCode.TX_SEND_ERR,
              ctx: {
                id,
                ixCount: built.ixCount,
                txSizeBytes: built.sizeBytes,
          ...
          try {
            emit("tx:history.updated", { id, status: "send_err" });
          } catch {}
        } catch {}
      ...
      try {
        ...
        await writeTxFullDump("execute", {
          ...
          plan,
          dexes: plan
            ? Array.from(new Set(plan.hops.map((h: any) => h.dex)))
            : [],
          exec: execCfg,
          execConfig: execCfg,
          built,
  ...
  api.post("/arb/execute/opportunity", async (req, res) => {
    try {
      const opp = req.body || {};
      const path: string[] = Array.isArray((opp as any)?.path)
        ? (opp as any).path
        : [];
      const hopPoolIds: string[] = Array.isArray((opp as any)?.hop_pool_ids)
        ? ((opp as any).hop_pool_ids as any[]).map((x: any) => String(x))
        : [];
      const dexes: string[] = Array.isArray((opp as any)?.hop_dexes)
        ? ((opp as any).hop_dexes as any[]).map((x: any) => String(x))
        : [];
      if (!Array.isArray(path) || path.length < 2)
        return res.status(400).json({ error: "invalid path" });
      ...
      if (hopPoolIds.length !== expected || dexes.length !== expected) {
        return res.status(400).json({
          error: `hop count mismatch: expected ${expected}, got poolIds=${hopPoolIds.length}, dexes=${dexes.length}`,
        });
      }
      ...
      req.body = {
        ...
        hopPoolIds,
        dexes,
        size: Number.isFinite((opp as any)?.size as any)
          ? Number((opp as any).size)
          : undefined,
        sizeUsd: Number.isFinite((opp as any)?.sizeUsd as any)
          ? Number((opp as any).sizeUsd)
          : undefined,
        slippageBps: Number.isFinite((opp as any)?.slippageBps as any)
          ? Number((opp as any).slippageBps)
          : undefined,
        forceDirect: (opp as any)?.forceDirect === true,
      } as any;
      return (api as any).handle(
        {
          ...req,
          url: "/arb/execute",
          originalUrl: "/arb/execute",
          path: "/arb/execute",
          method: "POST",
        },
        res,
  ...
  api.post("/arb/simulate-send/opportunity", async (req, res) => {
    try {
      ...
      return (api as any).handle(
        {
          ...req,
          url: "/arb/simulate-send",
          originalUrl: "/arb/simulate-send",
          path: "/arb/simulate-send",
          method: "POST",
        },
        res,
        () => {}
  ...
  api.post("/arb/execute/orca", async (req, res) => {
    try {
      try {
        logger.info("pretrade:arb execute orca start", {
          cat: "arb",
          code: LogCode.API_REQUEST,
          ctx: { body: req.body } as any,
        });
      } catch {}
      ...
      if (body && body.plan && Array.isArray(body.plan?.hops)) {
        ...
        const path: string[] = Array.isArray(body.path) ? body.path : [];
        const hopPoolIdsIn: string[] = Array.isArray(body.hopPoolIds)
          ? (body.hopPoolIds as any[]).map((x: any) => String(x))
          : [];
        const fallbackPid: string | undefined =
          body.poolId || body.whirlpoolId
            ? String(body.poolId || body.whirlpoolId)
            : undefined;
        const hopCount = Math.max(0, path.length - 1);

        try {
          logger.info("orca.execute.payload_construction", {
            cat: "arb",
            ctx: {
              pathLength: path.length,
              hopCount,
              hopPoolIdsInLength: hopPoolIdsIn.length,
              fallbackPid,
        ...
        if (hopCount <= 0) {
          try {
            logger.warn("orca.execute.invalid_path", {
              cat: "arb",
              ctx: { path, hopCount } as any,
            });
          } catch {}
          return res.status(400).json({ error: "invalid path" });
        }
        ...
        if (ids.length !== hopCount) {
          if (!fallbackPid) {
            try {
              logger.warn("orca.execute.missing_pool_id", {
                cat: "arb",
                ctx: { hopCount, hopPoolIdsInLength: ids.length, body } as any,
              });
            } catch {}
            return res
              .status(400)
              .json({ error: "missing hopPoolIds and poolId" });
          }
          ids = Array.from({ length: hopCount }, () => String(fallbackPid));
        }
        if (ids.some((s) => !s)) {
          try {
            logger.warn("orca.execute.invalid_pool_ids", {
              cat: "arb",
              ctx: { ids } as any,
            });
          } catch {}
        ...
        try {
          logger.info("orca.execute.payload_ready", {
            cat: "arb",
            ctx: {
              payloadPath: payload.path,
              payloadHopPoolIds: payload.hopPoolIds,
              payloadDexes: payload.dexes,
            } as any,
          });
        } catch {}
      }
      ...
      try {
        logger.error("orca.execute.error", {
          cat: "arb",
          code: LogCode.TX_BUILD_ERR,
          ctx: { error: String(e?.message || e), stack: e?.stack } as any,
        });
      } catch {}
      return res.status(400).json({ error: String(e?.message || e) });
    }
  ...
  api.post("/arb/simulate-send/orca", async (req, res) => {
    try {
      try {
        logger.info("pretrade:arb simulate-send orca start", {
          cat: "arb",
          code: LogCode.API_REQUEST,
          ctx: { body: req.body } as any,
        });
      ...
      if (body && body.plan && Array.isArray(body.plan?.hops)) {
        ...
        try {
          logger.info("orca.simulate.payload_construction", {
            cat: "arb",
            ctx: {
              pathLength: path.length,
              hopCount,
        ...
        if (hopCount <= 0) {
          try {
            logger.warn("orca.simulate.invalid_path", {
              cat: "arb",
              ctx: { path, hopCount } as any,
            });
          } catch {}
        ...
        if (ids.length !== hopCount) {
          if (!fallbackPid) {
            try {
              logger.warn("orca.simulate.missing_pool_id", {
                cat: "arb",
                ctx: { hopCount, hopPoolIdsInLength: ids.length, body } as any,
              });
            } catch {}
        ...
        if (ids.some((s) => !s)) {
          try {
            logger.warn("orca.simulate.invalid_pool_ids", {
              cat: "arb",
              ctx: { ids } as any,
            });
          } catch {}
        ...
        try {
          logger.info("orca.simulate.payload_ready", {
            cat: "arb",
            ctx: {
              payloadPath: payload.path,
              payloadHopPoolIds: payload.hopPoolIds,
      ...
      try {
        logger.error("orca.simulate.error", {
          cat: "arb",
          code: LogCode.TX_BUILD_ERR,
          ctx: { error: String(e?.message || e), stack: e?.stack } as any,
        });
    ...
    try {
      ...
      if (body && body.plan && Array.isArray(body.plan?.hops)) {
        ...
        const hopPoolIdsIn: string[] = Array.isArray(body.hopPoolIds)
          ? (body.hopPoolIds as any[]).map((x: any) => String(x))
          : [];
        const fallbackPid: string | undefined = body.poolId
          ? String(body.poolId)
          : undefined;
        const hopCount = Math.max(0, path.length - 1);
        if (hopCount <= 0)
  ...
  api.post("/arb/config", async (req, res) => {
    try {
      const host = process.env.ARB_SERVICE_URL || "http://127.0.0.1:4010";
      logger.debug(`api.request POST /arb-service/config`, {
        url: `${host}/config`,
        cat: "api",
      });
      const started = Date.now();
      ...
      try {
        delete toSend.edge_allow;
      } catch {}
      const r = await (async () => {
        const ac = new AbortController();
        const t = setTimeout(() => ac.abort("timeout"), 7000);
        try {
          return await fetch(`${host}/config`, {
            method: "POST",
            headers: { "content-type": "application/json" },
      ...
      logger.debug(
        `api.response POST /arb-service/config ${r.status} ${
          Date.now() - started
        }ms`,
        { status: r.status, cat: "api" }
      );
      let json: any = {};
      try {
        json = await r.json();
      ...
      try {
        ...
        emit("log", {
          ...
          message: `arb:config update keys=[${changedKeys.join(",")}] status=${
            r.status
          }`,
          timestamp: new Date().toISOString(),
          context: { cat: "arb" },
        ...
        emit("log", {
          level: r.ok ? "info" : "warn",
          message: `terminal: Arbitrage configuration ${
            r.ok ? "updated" : "update failed"
          } (${r.status})`,
          timestamp: new Date().toISOString(),
        });
      } catch {}
      try {
        await writeJson("backend/config/arbConfig.json", {
          ...(req.body || {}),
      ...
      try {
        ...
        (g as any).scheduleGraphRebuild?.(undefined, 50);
      } catch {}
      ...
      res.status(503).json({
        ok: false,
        error: "arb service unreachable; config saved locally",
      });
    }
  });

  function getArbBuildWorkerClient(): WorkerClient<
    ArbBuildRequest,
    ArbBuildResult
  > | null {
    ...
    try {
      ...
      arbBuildWorkerClient = createWorkerClient<
        ArbBuildRequest,
        ArbBuildResult
      >({
        url,
      ...
      try {
        logger.warn("arb.build.worker.init_failed", {
          error: String(err?.message || err),
          cat: "tx",
        });
      } catch {}
  ...
  function markArbBuildWorkerFailed(err: unknown): void {
    ...
    try {
      logger.debug("arb.build.worker.failed", {
        error: String((err as any)?.message || err),
        failures: arbBuildWorkerFailures,
        cat: "tx",
      });
    } catch {}
  }
  ...
  api.post("/arb/executor/start", async (req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.executor.api.start_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/executor/stop", async (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.executor.api.stop_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/executor/config", async (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.executor.api.config_read_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/executor/config", async (req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.executor.api.config_update_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/slippage/config", (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.slippage.api.config_read_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/slippage/config", (req: Request, res: Response) => {
    try {
      ...
      logger.info("arb.slippage.api.config_updated", {
        cat: "arb",
        updates: req.body,
        config: updated,
      });
      ...
      logger.error("arb.slippage.api.config_update_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/slippage/config/reset", (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.slippage.api.config_reset_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/jito/config", async (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.jito.api.config_read_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/jito/config", async (req: Request, res: Response) => {
    try {
      ...
      logger.info("arb.jito.api.config_updated", {
        cat: "arb",
        updates: req.body,
        config: updated,
      });
      ...
      logger.error("arb.jito.api.config_update_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/quarantine/status", (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.quarantine.api.status_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/quarantine/clear", async (_req: Request, res: Response) => {
    try {
      const { clearAllQuarantines } = await import(
        "../../execution/poolFailureTracker.js"
      );
      const count = clearAllQuarantines();
      logger.info("arb.quarantine.api.cleared", { cat: "arb", count });
      res.json({ status: "cleared", count });
    } catch (e: any) {
      logger.error("arb.quarantine.api.clear_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.delete("/arb/quarantine/:poolId", async (req: Request, res: Response) => {
    try {
      ...
      const { removeFromQuarantine } = await import(
        "../../execution/poolFailureTracker.js"
      );
      ...
      if (removed) {
        ...
        res.json({
          status: "not_found",
          poolId,
          message: "Pool was not quarantined",
        });
      }
    } catch (e: any) {
      logger.error("arb.quarantine.api.remove_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/blocklist/add", async (req: Request, res: Response) => {
    try {
      ...
      if (!poolId || typeof poolId !== "string") {
        res
          .status(400)
          .json({ error: "poolId (string) is required in request body" });
        return;
      }

      const { addToManualBlocklist, getManualBlocklist } = await import(
        "../../execution/poolFailureTracker.js"
      );
      ...
      const blocklist = new Set<string>(
        currentConfig.manualPoolBlocklist || []
      );
      ...
      logger.error("arb.blocklist.api.add_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/blocklist/remove", async (req: Request, res: Response) => {
    try {
      ...
      const { removeFromManualBlocklist, getManualBlocklist } = await import(
        "../../execution/poolFailureTracker.js"
      );
      ...
      logger.error("arb.blocklist.api.remove_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/blocklist", async (_req: Request, res: Response) => {
    try {
      const { getManualBlocklist } = await import(
        "../../execution/poolFailureTracker.js"
      );
      res.json({ blocklist: getManualBlocklist() });
    } catch (e: any) {
      logger.error("arb.blocklist.api.get_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/poolsub/status", async (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.poolsub.api.status_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/poolsub/clear", async (_req: Request, res: Response) => {
    try {
      const { clearSubstitutions } = await import(
        "../../execution/poolSubstitutionStore.js"
      );
      clearSubstitutions();
      res.json({ success: true });
    } catch (e: any) {
      logger.error("arb.poolsub.api.clear_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/poolsub/prune", async (_req: Request, res: Response) => {
    try {
      const { pruneExpired } = await import(
        "../../execution/poolSubstitutionStore.js"
      );
      const pruned = pruneExpired();
      res.json({ success: true, pruned });
    } catch (e: any) {
      logger.error("arb.poolsub.api.prune_failed", {
        cat: "arb",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/alt/config", async (_req: Request, res: Response) => {
    try {
      const { loadAltConfig, saveAltConfig } = await import(
        "../../execution/utils/altConfig.js"
      );
      const { dexAltManager } = await import(
        "../../execution/utils/altManager.js"
      );
      ...
      res.json({
        ...
        cache: {
          ...
          totalAccounts: cachedAlts.reduce(
            (sum, alt) => sum + (alt.state?.addresses?.length || 0),
            0
          ),
      ...
      logger.error("arb.alt.api.config_read_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alt/create/common", async (_req: Request, res: Response) => {
    try {
      ...
      const address = await (dexAltManager as any).createAltOnChain(
        wallet,
        accounts,
        "common"
      );
      ...
      logger.error("arb.alt.api.common_create_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alt/create/flashloan", async (req: Request, res: Response) => {
    try {
      ...
      const routerProgramId = req.body.routerProgramId
        ...
        : new PublicKey(
            (CONFIG as any)?.router?.programId ||
      ...
      const accounts = await dexAltManager.collectFlashloanAccounts(
        vaultOwner,
        routerProgramId
      );
      const address = await (dexAltManager as any).createAltOnChain(
      ...
      logger.error("arb.alt.api.flashloan_create_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post(
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        const result = await dexAltManager.refreshUserPdaAltShards({
          allowCreate: true,
        });
        ...
        logger.error("arb.alt.api.userPdas_create_failed", {
          cat: "tx",
          error: String(e?.message || e),
        });
        res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post(
    "/arb/alt/extend-auto/common",
    async (_req: Request, res: Response) => {
      try {
        const { dexAltManager } = await import(
          "../../execution/utils/altManager.js"
        );
        const { loadAltConfig } = await import(
          "../../execution/utils/altConfig.js"
        );
        const { ensureWallet } = await import("../../wallet/wallet.js");
        ...
        if (!config.alts.common) {
          return res.status(400).json({
            error:
              "Common ALT does not exist. Create it first using /arb/alt/create/common",
          });
        }

        // Collect fresh common accounts
        const allAccounts = await (dexAltManager as any).collectCommonAccounts(
          wallet.publicKey
        );

        // Get existing accounts in the ALT
        const existingAlt = dexAltManager.getCachedAltByAddress(
          config.alts.common
        );
        if (!existingAlt) {
          return res.status(400).json({
        ...
        logger.error("arb.alt.api.common_extend_failed", {
          cat: "tx",
          error: String(e?.message || e),
        });
        res.status(500).json({ error: String(e?.message || e) });
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        if (!config.userPdaAlts?.addresses?.length && !config.alts.userPdas) {
          return res.status(400).json({
            error:
              "User PDA ALTs do not exist. Create them first using /arb/alt/create/user-pdas",
          });
        }
        ...
        logger.error("arb.alt.api.userPdas_extend_failed", {
          cat: "tx",
          error: String(e?.message || e),
        });
        res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alt/create/dex-pools", async (req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.alt.api.dex_pools_create_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alt/coverage", async (req: Request, res: Response) => {
    try {
      const { analyzeRouteAlts, DexType } = await import(
        "../../execution/utils/altSelection.js"
      );

      const poolIds = req.body.poolIds as string[];
      const dexTypes =
        (req.body.dexTypes as number[]) || poolIds.map(() => DexType.Raydium);
      ...
      logger.error("arb.alt.api.coverage_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
    ...
    try {
      ...
      if (poolIds && poolIds.length > 0) {
        ...
        for (const poolId of poolIds) {
          const poolAccounts = await (
            dexAltManager as any
          ).collectPoolSpecificAccounts(poolId, dex);
          collectedAccounts.push(
            ...poolAccounts.map((pk: any) => pk.toBase58())
          );
        }

        const result = await dexAltManager.extendAlt(
          category,
          collectedAccounts
        );
      ...
      logger.error("arb.alt.api.extend_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alt/refresh", async (_req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.alt.api.refresh_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/alt/pools", async (req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.alt.api.pools_read_failed", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alts/create-all", async (req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.alt.api.create_all.error", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/arb/alts/coverage", async (req: Request, res: Response) => {
    try {
      ...
      if (snapshot?.edges) {
        for (const edge of snapshot.edges) {
          ...
          if (
            ...
            if (poolKind === "cpmm" || dex.includes("cpmm"))
              dexPools["raydium-cpmm"].add(cleanPoolId);
            else if (poolKind === "amm" || dex.includes("amm"))
              dexPools["raydium-amm"].add(cleanPoolId);
      ...
      const coverage: Record<
        string,
        { total: number; covered: number; percent: string }
      > = {};
      ...
      for (const [dex, pools] of Object.entries(dexPools)) {
        ...
        coverage[dex] = {
          ...
          percent:
            pools.size > 0
              ? `${((covered / pools.size) * 100).toFixed(1)}%`
              : "N/A",
        };
      }

      // Total coverage
      const allPools = Object.values(dexPools).reduce(
      ...
      res.json({
        total: {
          ...
          percent:
            allPools > 0
              ? `${((allCovered / allPools) * 100).toFixed(1)}%`
              : "N/A",
        },
        byDex: coverage,
        alts: {
        ...
        dexAlts: {
          ...
          "raydium-amm":
            config.dexAlts?.["raydium-amm"]?.addresses?.length || 0,
          "raydium-cpmm":
            config.dexAlts?.["raydium-cpmm"]?.addresses?.length || 0,
          orca: config.dexAlts?.orca?.addresses?.length || 0,
          meteora: config.dexAlts?.meteora?.addresses?.length || 0,
          "meteora-balanced":
            config.dexAlts?.["meteora-balanced"]?.addresses?.length || 0,
          "meteora-damm-v1":
            config.dexAlts?.["meteora-damm-v1"]?.addresses?.length || 0,
          "meteora-damm-v2":
            config.dexAlts?.["meteora-damm-v2"]?.addresses?.length || 0,
          pumpswap: config.dexAlts?.pumpswap?.addresses?.length || 0,
        },
      });
    } catch (e: any) {
      logger.error("arb.alt.api.coverage.error", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alts/extend/:dex", async (req: Request, res: Response) => {
    try {
      ...
      if (!validDexes.includes(dex)) {
        return res.status(400).json({
          error: `Invalid DEX. Must be one of: ${validDexes.join(", ")}`,
        });
      }
      ...
      const result = await dexAltManager.createAllDexPoolAlts(
        dex as
          | "raydium"
          | "raydium-amm"
          | "raydium-cpmm"
      ...
      logger.error("arb.alt.api.extend.error", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/arb/alts/common", async (req: Request, res: Response) => {
    try {
      ...
      logger.error("arb.alt.api.common.error", {
        cat: "tx",
        error: String(e?.message || e),
      });
      res.status(500).json({ error: String(e?.message || e) });
   ...
   */
  api.post(
    "/arb/alts/deactivate-by-address",
