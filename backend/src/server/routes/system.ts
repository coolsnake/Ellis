...
export function createSystemRouter(_io: SocketIOServer): Router {
  ...
  api.get("/system/config", async (_req, res) => {
    try {
      ...
      try {
        const { isPoolDataValidationEnabled, isReactiveValidationRunning } =
      ...
      res.json({
        ...
        pools: {
          activationMode:
            (CONFIG.system as any)?.poolActivationMode || "immediate",
      ...
      logger.error("server: failed to get system config", {
    ...
    try {
      const {
        ...
        sanity?: {
          ...
          sampleRate?: number;
          sanity_applyRaydiumAmm?: boolean;
          sanity_applyOrcaClmm?: boolean;
        };
      ...
      if (system) {
        ...
        try {
          if (Array.isArray(system.enabledLogCategories)) {
            nextSystem.enabledLogCategories = (
              system.enabledLogCategories as string[]
            ).map((s) => String(s).toLowerCase());
          }
          if (Array.isArray(system.frontendEnabledLogCategories)) {
            nextSystem.frontendEnabledLogCategories = (
              system.frontendEnabledLogCategories as string[]
            ).map((s) => String(s).toLowerCase());
          }
          if (
            Array.isArray(nextSystem.enabledLogCategories) &&
            nextSystem.enabledLogCategories.length
          ) {
            nextSystem.log = nextSystem.log || {};
            const cats: Record<string, "error" | "warn" | "info" | "debug"> = {
              ...(nextSystem.log?.categories || {}),
            };
            const on = new Set<string>(nextSystem.enabledLogCategories);
            const all: string[] = Array.isArray(nextSystem.logCategories)
              ? nextSystem.logCategories
              : [];
            for (const c of all) {
        ...
        if (
          ...
          try {
            const { saveRuntimeConfig } = await import(
              "../runtimeConfigStore.js"
            );
            await saveRuntimeConfig({
              jupiterApiKey: String(system.jupiterTopTokens.apiKey || ""),
            });
          } catch (e: any) {
            logger.warn("server: failed to persist runtime config", {
              error: String(e?.message || e),
              cat: "server",
            });
          }
      ...
      if (raydiumCpmm)
        (CONFIG as any).raydiumCpmm = {
          ...(CONFIG as any).raydiumCpmm,
          ...raydiumCpmm,
        };
      if (raydiumClmm)
        (CONFIG as any).raydiumClmm = {
          ...(CONFIG as any).raydiumClmm,
          ...raydiumClmm,
        };
      if (orca) CONFIG.orca = { ...CONFIG.orca, ...orca } as any;
      if (meteora)
        (CONFIG as any).meteora = {
          ...(CONFIG as any).meteora,
          ...meteora,
        } as any;
      if (meteoraBalanced)
        (CONFIG as any).meteoraBalanced = {
          ...(CONFIG as any).meteoraBalanced,
          ...meteoraBalanced,
        } as any;
      if (pumpswap)
        (CONFIG as any).pumpswap = {
          ...(CONFIG as any).pumpswap,
          ...pumpswap,
        } as any;
      if (sanity)
        (CONFIG as any).sanity = {
          ...(CONFIG as any).sanity,
          ...sanity,
        } as any;
      ...
      if (pools?.activationMode) {
        try {
          ...
          logger.info("server: pool activation mode changed", {
            mode: pools.activationMode,
            enabled,
            cat: "server",
          });
        } catch (activationErr) {
          logger.warn("server: failed to set pool activation mode", {
            error: String((activationErr as Error)?.message || activationErr),
            cat: "server",
          });
        }
      }
      ...
      if (validation?.poolDataValidationEnabled !== undefined) {
        try {
          const {
            setPoolDataValidationEnabled,
            startReactiveValidation,
            isReactiveValidationRunning,
          } = await import("../../execution/cacheValidator.js");
          ...
          if (enabled && !isReactiveValidationRunning()) {
            const intervalMs = Math.max(
              250,
              Number(
                (CONFIG.system as any)?.tickArrayValidatorIntervalMs || 500
              )
            );
            startReactiveValidation(intervalMs);
          }

          logger.info("server: pool data validation toggled", {
            enabled,
            cat: "server",
          });
        } catch (validationErr) {
      ...
      if (discovery) {
        ...
        if (typeof discovery.enabled === "boolean")
          disc.enabled = discovery.enabled;
        if (
          typeof discovery.intervalMs === "number" &&
          discovery.intervalMs > 0
        )
          disc.intervalMs = discovery.intervalMs;
        if (typeof discovery.minLiquidityUsd === "number")
          disc.minLiquidityUsd = Math.max(0, discovery.minLiquidityUsd);
        if (typeof discovery.maxPoolsPerToken === "number")
          disc.maxPoolsPerToken = Math.max(1, discovery.maxPoolsPerToken);
        (CONFIG as any).discovery = disc;
        logger.info("server: discovery config updated", {
      ...
      try {
        if (
          system &&
          Object.prototype.hasOwnProperty.call(system, "enableLogging")
        ) {
        ...
        if (
          system &&
          (Object.prototype.hasOwnProperty.call(system, "logToFile") ||
            typeof system.logFilePath === "string")
        ) {
      ...
      logger.error("server: failed to update system config", {
        error: String(e?.message || e),
        cat: "server",
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/system/shutdown", async (_req, res) => {
    try {
      emit("log", {
        level: "warn",
        message: "terminal: shutdown requested from UI",
        timestamp: new Date().toISOString(),
      });
      res.json({ ok: true });
      ...
      setTimeout(async () => {
        try {
          ...
          try {
            emit("log", {
              level: "error",
              message: `terminal: shutdown error: ${String(e?.message || e)}`,
              timestamp: new Date().toISOString(),
            });
          } catch {}
          process.exit(1);
  ...
  api.get("/system/rpc/metrics", async (_req, res) => {
    try {
      ...
      logger.error("server: failed to get RPC metrics", {
        error: String(e?.message || e),
        cat: "server",
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.get("/system/rpc/limiter/config", async (_req, res) => {
    try {
      const { loadRpcLimiterConfig } = await import(
        "../rpcLimiterConfigStore.js"
      );
      const config = await loadRpcLimiterConfig();
      res.json(config);
    } catch (e: any) {
      logger.error("server: failed to get RPC limiter config", {
        error: String(e?.message || e),
        cat: "server",
      });
      res.status(500).json({ error: String(e?.message || e) });
  ...
  api.post("/system/rpc/limiter/config", async (req, res) => {
    try {
      ...
      if (maxRps !== undefined && (!Number.isFinite(maxRps) || maxRps <= 0)) {
        return res
          .status(400)
          .json({ error: "maxRps must be a positive number" });
      }
      if (burst !== undefined && (!Number.isFinite(burst) || burst < 0)) {
        return res
          .status(400)
          .json({ error: "burst must be a non-negative number" });
      }
      if (
        minGapMs !== undefined &&
        (!Number.isFinite(minGapMs) || minGapMs < 0)
      ) {
        return res
          .status(400)
          .json({ error: "minGapMs must be a non-negative number" });
      }

      // Update the in-memory limiter first
      const { updateRpcLimiterConfig } = await import(
        "../../utils/rpcLimiter.js"
      );
      ...
      // Persist to file
      const { saveRpcLimiterConfig, loadRpcLimiterConfig } = await import(
        "../rpcLimiterConfigStore.js"
      );
      ...
      logger.error("server: failed to update RPC limiter config", {
        error: String(e?.message || e),
        cat: "server",
      });
      res.status(500).json({ error: String(e?.message || e) });
