...
import {
  pushArbGraphSnapshot,
  pushArbGraphDiff,
} from "./graphPushOrchestrator.js";
import {
  shouldPushGraphUpdate,
  logPushDecision,
} from "./graph.push.coordinator.js";
import { CONFIG } from "../utils/config.js";
import {
  getRaydiumPoolsNormalized,
  getOrcaPoolsCached,
  enablePoolWebsocketRefreshes,
  peekMeteoraPools,
  getMeteoraPoolsCached,
...
type GraphEdgeLite = Pick<
  GraphEdge,
  "id" | "source" | "target" | "dex" | "pool_id" | "weight"
>;
...
const REBASE_DIFF_THRESHOLD = Math.max(
  0,
  Number((CONFIG.system as any)?.graphRebaseDiffThreshold || 2000)
);
const REBASE_TIME_MS = Math.max(
  0,
  Number((CONFIG.system as any)?.graphRebaseTimeMs || 5 * 60 * 1000)
);
let lastRebaseMs = 0;
let lastRebuildMs = 0;
let lastRebuildHadChanges = false;
const MIN_REBUILD_GAP_MS = Math.max(
  100,
  Number((CONFIG.system as any)?.graphMinRebuildGapMs || 500)
);
...
const GRAPH_WORKER_DISABLED =
  String(
    env.GRAPH_WORKER_DISABLED ?? env.ARB_GRAPH_WORKER_DISABLED ?? ""
  ).toLowerCase() === "true";
const GRAPH_WORKER_MAX_QUEUE = Math.max(
  1,
  Number(env.GRAPH_WORKER_MAX_QUEUE ?? env.ARB_GRAPH_WORKER_MAX_QUEUE ?? 4)
);
const GRAPH_WORKER_TIMEOUT_MS = Math.max(
  2000,
  Number(env.GRAPH_WORKER_TIMEOUT_MS ?? env.ARB_GRAPH_WORKER_TIMEOUT_MS ?? 8000)
);

const GRAPH_WORKER_IDLE_MS = Math.max(
  0,
  Number(env.GRAPH_WORKER_IDLE_MS ?? env.ARB_GRAPH_WORKER_IDLE_MS ?? 60_000)
);
const GRAPH_WORKER_CONCURRENCY = Math.max(
  1,
  Number(env.GRAPH_WORKER_CONCURRENCY ?? env.ARB_GRAPH_WORKER_CONCURRENCY ?? 1)
);

let graphWorkerClient: WorkerClient<
  GraphWorkerRequest,
  GraphWorkerResponse
> | null = null;
...
export function dropPoolRuntime(id: string): {
  added: boolean;
  current: string[];
} {
  ...
  try {
    logger.info("graph.drop.runtime", {
      pool_id: key,
      total: droppedPoolIds.size,
    });
  } catch {}
  return {
    added: droppedPoolIds.size > before,
    current: Array.from(droppedPoolIds),
  };
}
...
async function loadEdgeAllow(): Promise<EdgeAllow> {
  try {
    ...
    return cfg && (cfg as any).edge_allow
      ? ((cfg as any).edge_allow as EdgeAllow)
      : ({} as EdgeAllow);
  } catch {
    return {} as EdgeAllow;
  }
...
export function getGraphVersion(): { version: number; timestamp: number } {
  ...
  try {
    logger.debug("graph.version.peek", {
      version,
      timestamp,
      cat: "graph",
      code: LogCode.GRAPH_TVL_STATS,
    });
  } catch {}
...
async function applyUpdateWithLock(
...
async function rebuildGraphNowInternal(
  io?: SocketIOServer,
  opts?: { pushToArb?: boolean; source?: string }
): Promise<void> {
  try {
    ...
    if (
      ...
      if (gap < MIN_REBUILD_GAP_MS) {
        try {
          logger.debug("graph.rebuild.skip_recent_no_change", {
            gap_ms: gap,
            min_gap_ms: MIN_REBUILD_GAP_MS,
            cat: "graph",
          });
        } catch {}
        return;
    ...
    if (
      !prev &&
      (!next ||
        !Array.isArray((next as any).edges) ||
        (next as any).edges.length === 0)
    ) {
      try {
        logger.info("graph.rebuild.skip_empty", {
    ...
    if (io) {
      if (!prev)
        io.emit("graph-rebase", {
          version: next.version,
          timestamp: next.timestamp,
        });
      else if (changed) io.emit("graph-update", toLiteDiff(diff));
    } else {
      try {
        if (!prev)
          emit("graph-rebase", {
        ...
        else if (changed) emit("graph-update", toLiteDiff(diff));
      } catch {}
    }
    ...
    if (!prev) {
      if (decision.shouldPush) {
        try {
          void pushArbGraphSnapshot(next);
        } catch {}
      }
      diffSinceRebase = 0;
      ...
      if (decision.shouldPush) {
        ...
        const shouldRebase =
          ...
          nowMs - lastRebaseMs > REBASE_TIME_MS;
        if (shouldRebase) {
          try {
            void pushArbGraphSnapshot(next);
          } catch {}
          ...
          try {
            emit("log", {
              level: "info",
              message: `graph:push rebase v=${next.version} nodes=${next.nodes.length} edges=${next.edges.length}`,
              timestamp: new Date().toISOString(),
              context: { cat: "graph", code: LogCode.GRAPH_PUSH_SNAPSHOT },
            });
          } catch {}
          try {
            if (io)
          ...
          try {
            emit("log", {
              level: "info",
              message: `graph:push diff v=${diff.version} changes=${
                diff.addedEdges.length +
                diff.updatedEdges.length +
                diff.removedEdgeIds.length
              }`,
              timestamp: new Date().toISOString(),
              context: { cat: "graph", code: LogCode.GRAPH_PUSH_DIFF },
            });
          } catch {}
        }
      }
    ...
    logger.debug("graph.rebuild.now failed", {
      error: String(e?.message || e),
    });
  }
}

export async function rebuildGraphNow(
  io?: SocketIOServer,
  opts?: { pushToArb?: boolean }
): Promise<void> {
...
export function scheduleGraphRebuild(
  io?: SocketIOServer,
  debounceMs = 200
): void {
  if (rebuildTimer) {
  ...
  const minDebounce = Math.max(
    5,
    Number((CONFIG.system as any)?.graphRebuildMinDebounceMs || 50)
  );
  ...
  rebuildTimer = setTimeout(() => {
    ...
    pendingUpdates = 0;
    try {
      logger.debug("graph.rebuild.batch", {
        pending,
        code: LogCode.GRAPH_REBUILD_BATCH,
      });
    } catch {}
    rebuildGraphNow(io).catch(() => {});
  }, wait);
  try {
    logger.debug("graph.rebuild.scheduled", {
      debounceMs,
      code: LogCode.GRAPH_REBUILD_SCHEDULED,
    });
  } catch {}
}

// Incremental graph apply from pool deltas (diff-first, occasional rebase)
const priceClampMinInc = Number(
  ((CONFIG as any)?.sanity as any)?.priceClampMin ?? 1e-12
);
const priceClampMaxInc = Number(
  ((CONFIG as any)?.sanity as any)?.priceClampMax ?? 1e12
);
  ...
  return applyUpdateWithLock(async () => {
    try {
      ...
      if (worker && totalPending < GRAPH_WORKER_MAX_QUEUE) {
        try {
          usedWorker = true;
          result = await worker.run(workerRequest, {
            timeoutMs: GRAPH_WORKER_TIMEOUT_MS,
          });
        } catch (err: any) {
          usedWorker = false;
          markGraphWorkerFailed(err);
          try {
            logger.warn("graph.worker.incremental_failed", {
        ...
        if (worker && totalPending >= GRAPH_WORKER_MAX_QUEUE) {
          try {
            logger.warn("graph.worker.queue_saturated", {
              queue: queueDepth,
              active,
              limit: GRAPH_WORKER_MAX_QUEUE,
              cat: "graph",
            });
          } catch {}
        }
      ...
      try {
        logger.debug("graph.incremental.compute", {
          worker: usedWorker,
      ...
      if (shouldRebase) {
        ...
        if (decision.shouldPush) {
          try {
            void pushArbGraphSnapshot(result.snapshot);
          } catch {}
        }
      } else {
        diffSinceRebase += ch;
        ...
        try {
          emit("graph-update", toLiteDiff(diff));
        } catch {}
        if (decision.shouldPush) {
          try {
            void pushArbGraphDiff(diff);
      ...
      try {
        logger.warn("graph.incremental.apply failed", {
          error: String(e?.message || e),
          stack: e?.stack,
          cat: "graph",
        });
...
function getGraphWorkerClient(): WorkerClient<
  GraphWorkerRequest,
  GraphWorkerResponse
> | null {
  ...
  try {
    ...
    graphWorkerClient = createWorkerClient<
      GraphWorkerRequest,
      GraphWorkerResponse
    >({
      url,
    ...
    try {
      logger.warn("graph.worker.init_failed", {
        error: String(err?.message || err),
        cat: "graph",
      });
    } catch {}
...
function markGraphWorkerFailed(err: unknown): void {
  ...
  try {
    logger.debug("graph.worker.disabled_temp", {
      error: String((err as any)?.message || err),
      failures: graphWorkerFailureCount,
      cat: "graph",
    });
  } catch {}
}
...
function buildPriceMap(
  ...
  const getter =
    typeof priceStore?.getPriceByMint === "function"
      ? priceStore.getPriceByMint.bind(priceStore)
      : undefined;
  ...
  const addMint = (mint: unknown) => {
    ...
    try {
      ...
      const val = Number(
        (info as any)?.usdc ?? (info as any)?.usd ?? (info as any)?.price
      );
...
let lastPushStart = 0;
export function markGraphBuildStart(): void {
  lastBuildStart = Date.now();
}
export function markGraphPushStart(): void {
  lastPushStart = Date.now();
}
export function getGraphTimings(): {
  graph_build_ms?: number;
  graph_push_latency_ms?: number;
} {
...
export async function getGraphSnapshot(force = false): Promise<GraphSnapshot> {
  const now = Date.now();
  if (!force && lastSnapshot && now - lastAt < getSnapshotTtlMs())
    return lastSnapshot;
  ...
  inflight = (async () => {
    try {
      ...
      const rayRaw =
        overrides?.raydium ??
        (typeof poolsMod.peekRaydiumPools === "function"
          ? poolsMod.peekRaydiumPools()
          : { amm: [], clmm: [], cpmm: [] });
      const orcRaw =
        overrides?.orca ??
        (typeof poolsMod.peekOrcaPools === "function"
          ? poolsMod.peekOrcaPools()
          : { amm: [], clmm: [], cpmm: [] });
      const metRaw =
        overrides?.meteora ??
        (typeof poolsMod.peekMeteoraPools === "function"
          ? poolsMod.peekMeteoraPools()
          : { amm: [], clmm: [], cpmm: [] });
      const mblRaw =
        overrides?.meteora_balanced ??
        (typeof poolsMod.peekMeteoraBalancedPools === "function"
          ? poolsMod.peekMeteoraBalancedPools()
          : { amm: [], clmm: [], cpmm: [] });
      const pumpRaw =
        overrides?.pumpswap ??
        (typeof poolsMod.peekPumpswapPools === "function"
          ? poolsMod.peekPumpswapPools()
          : { amm: [], clmm: [], cpmm: [] });
      ...
      try {
        ...
        if (count <= 0) {
          try {
            logger.debug("graph.snapshot.skip", {
              reason: "no_retained_pools",
            });
          } catch {}
      ...
      const tokenMap = await loadTokenMap().catch(
        () => ({} as Record<string, { mint: string; decimals: number }>)
      );
      ...
      try {
        ...
        for (const [mint, meta] of Object.entries(jupiterMap)) {
          ...
          if (
            Number.isFinite((meta as any)?.decimals) &&
            decimalsByMint[mint] == null
          )
            decimalsByMint[mint] = Number((meta as any).decimals);
        }
        try {
          logger.debug("graph.labels.jupiter", {
            loaded: Object.keys(jupiterMap).length,
            labelsAdded: jupiterLabelsAdded,
            cat: "graph",
          });
        } catch {}
      } catch (e: any) {
        try {
          logger.warn("graph.labels.jupiter.failed", {
            error: String(e?.message || e),
            cat: "graph",
          });
        } catch {}
      ...
      let clmmTotal = 0,
        clmmUsd = 0,
        clmmMissingAmounts = 0,
        clmmMissingDecimals = 0;
      ...
      const feeMin = Number.isFinite(Number(sanityCfg.feeMin))
        ? Number(sanityCfg.feeMin)
        : 0;
      const feeMax = Number.isFinite(Number(sanityCfg.feeMax))
        ? Number(sanityCfg.feeMax)
        : 10000;
      const maxPriceDeviation = Number.isFinite(
        Number(sanityCfg.maxPriceDeviation)
      )
        ? Number(sanityCfg.maxPriceDeviation)
        : 10;
      ...
      const validationConfig = {
        ...
        maxPriceDeviation,
        sanityEnabled:
          sanityEnabled && (sanityCfg as any).applyAtGraph !== false,
      };
      ...
      const getPriceByMintVar = (m: string) => {
        try {
          return priceStore.getPriceByMint(m);
        } catch {
      ...
      const getUsd = (m: string): number | undefined => {
        try {
          return getPriceByMintVar(m)?.usdc ?? undefined;
        } catch {
          return undefined;
        }
      };
      ...
      const validatePoolsForGraph = (norm: NormPools): NormPools => {
        ...
        const drop = {
          ...
          priceOutliers: 0,
          nonFinitePrice: 0,
          amm: { total: 0, dropped: 0 },
          clmm: { total: 0, dropped: 0 },
          cpmm: { total: 0, dropped: 0 },
        } as any;
        ...
        for (const p of norm.amm || []) {
          if (checkPool(p, "amm")) out.amm.push(p);
        }
        for (const p of norm.clmm || []) {
          if (checkPool(p, "clmm")) out.clmm.push(p);
        }
        for (const p of norm.cpmm || []) {
          if (checkPool(p, "cpmm")) out.cpmm!.push(p);
        }
        ...
        try {
          logger.info("graph.sanity.filter", {
            ...
            dropped: drop,
            before: {
              amm: norm.amm?.length || 0,
              clmm: norm.clmm?.length || 0,
              cpmm: norm.cpmm?.length || 0,
            },
            after: {
              amm: out.amm.length,
              clmm: out.clmm.length,
              cpmm: out.cpmm?.length || 0,
            },
            cat: "graph",
          });
        } catch {}
      ...
      const createEdgesFromPools = (pools: any[], dex: string) => {
        ...
        let edgesCreated = 0;
        const poolKind =
          pools.length > 0 ? (pools[0] as any)?.pool_kind : "unknown";
        ...
        for (const p of pools) {
          ...
          for (const e of newEdges) {
            ...
            if (!nodesMap[e.source]) {
              nodesMap[e.source] = {
                id: e.source,
                label: getLabelForMint(e.source),
              };
            }
            if (!nodesMap[e.target]) {
              nodesMap[e.target] = {
                id: e.target,
                label: getLabelForMint(e.target),
              };
            }
          }
        }
      ...
      try {
        logger.info("graph.pools.after_validation", {
          raydium: {
            amm: rayValid.amm?.length || 0,
            clmm: rayValid.clmm?.length || 0,
            cpmm: rayValid.cpmm?.length || 0,
          },
          orca: {
            amm: orcValid.amm?.length || 0,
            clmm: orcValid.clmm?.length || 0,
          },
      ...
      try {
        ...
        const labeledCount = Array.from(totalMints).filter(
          (m) => labelByMint[m]
        ).length;
      ...
      try {
        ...
        for (const p of allPools) {
          ...
          if (!dex) {
            poolsWithoutDex.push(
              String((p as any)?.id || "unknown").slice(0, 12)
            );
          }
          poolDexCounts[dex || "MISSING"] =
            (poolDexCounts[dex || "MISSING"] || 0) + 1;
        }
      ...
      try {
        ...
        try {
          ...
          const amm = all.find(
            (e: any) =>
              e.dex === "Raydium" &&
              e.pool_kind === "amm" &&
              ((e.source === USDC && e.target === SOL) ||
                (e.source === SOL && e.target === USDC))
          );
          const clmm = all.find(
            (e: any) =>
              e.dex === "Raydium" &&
          ...
          if (amm && clmm && amm.price_a_per_b && clmm.price_a_per_b) {
            const ratio =
              Number(clmm.price_a_per_b) / Number(amm.price_a_per_b);
        ...
        try {
          ...
          if (usdcSol) {
            const nodes = new Set<string>(
              all.flatMap((e: any) => [e.source, e.target])
            );
            for (const X of nodes) {
              ...
              if (usdcX && xSol) {
                const product =
                  (usdcX.price_a_per_b as number) *
                  (xSol.price_a_per_b as number);
                const ref = usdcSol.price_a_per_b as number;
                const dev = Math.max(product / ref, ref / product);
                if (!(dev < 2.0)) {
                  logger.debug("graph.diagnostic.triangle", {
                    X,
                    usdcX: usdcX.id,
                    xSol: xSol.id,
                    usdcSol: usdcSol.id,
        ...
        const countDex = (dex: string) =>
          all.filter((e: any) => e.dex === dex).length;
        const sample = (dex: string) =>
          all
            ...
            .map((e: any) => ({
              ...
              source: e.source,
              target: e.target,
              pool_id: (e as any)?.pool_id,
              kind: (e as any)?.pool_kind,
        ...
        logger.debug("graph.edges.count", {
          ...
          meteoraBalanced_v1: {
            count: countDex("MeteoraBalanced_v1"),
            sample: sample("MeteoraBalanced_v1"),
          },
          meteoraBalanced_v2: {
            count: countDex("MeteoraBalanced_v2"),
            sample: sample("MeteoraBalanced_v2"),
          },
        ...
        if (
          orcaCount === 0 &&
          (orcValid.amm?.length || 0) + (orcValid.clmm?.length || 0) > 0
        ) {
          try {
            logger.warn("graph.orca.missing_edges", {
              orcaValidPools:
                (orcValid.amm?.length || 0) + (orcValid.clmm?.length || 0),
              orcaEdges: orcaCount,
        ...
        const mblCount =
          countDex("MeteoraBalanced_v1") + countDex("MeteoraBalanced_v2");
      ...
      try {
        ...
        const pick = (kind: "amm" | "clmm") => {
          ...
          return sample.map((e) => ({
            ...
            source: e.source,
            target: e.target,
            pool_kind: (e as any).pool_kind,
            direction: (e as any).direction,
            fee_bps: e.fee_bps,
            price_a_per_b: e.price_a_per_b,
            inverse_per_source:
              (e as any)?.price_a_per_b && (e as any).price_a_per_b > 0
                ? 1 / (e as any).price_a_per_b
                : undefined,
            tvl_usd: e.tvl_usd,
        ...
        logger.debug("graph.edges sample", {
          amm: sampleAmm,
          clmm: sampleClmm,
          cat: "graph",
        });
      ...
      try {
        logger.debug("graph.consistency.summary", {
          ray: consistency.ray,
          orc: consistency.orc,
          met: consistency.met,
          cat: "graph",
      ...
      try {
        const { validateEdgesSimple } = await import(
      ...
      try {
        logger.debug("graph.tvl.stats", {
          amm: { total: ammTotal, usd: ammUsd },
          clmm: {
            total: clmmTotal,
            usd: clmmUsd,
            missingAmounts: clmmMissingAmounts,
            missingDecimals: clmmMissingDecimals,
          },
        });
      } catch {}
...
export function startGraphStream(io: SocketIOServer): void {
  ...
  try {
    logger.debug("graph.stream.disabled", {
      reason: "event_driven_only",
      cat: "graph",
    });
  } catch {}
}
