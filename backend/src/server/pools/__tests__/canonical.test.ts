...
import { canonicalOrientation, canonicalizePools, swapPoolFields, clearCanonicalCache } from '../canonical.js';
...
describe('Canonical Orientation', () => {
  ...
  describe('canonicalOrientation', () => {
    ...
    it('should handle quote hierarchy correctly', () => {
      // USDC has highest priority, should always be on B
      expect(canonicalOrientation(USDT, USDC)).toBe('swap'); // USDC to B
      expect(canonicalOrientation(USDC, USDT)).toBe('keep'); // USDC already on B
      expect(canonicalOrientation(SOL, USDC)).toBe('swap'); // USDC to B
      expect(canonicalOrientation(USDC, SOL)).toBe('keep'); // USDC already on B
      
      // USDT has second priority
      expect(canonicalOrientation(SOL, USDT)).toBe('swap'); // USDT to B
      expect(canonicalOrientation(USDT, SOL)).toBe('keep'); // USDT already on B
      
      // USD1 has third priority
      expect(canonicalOrientation(SOL, USD1)).toBe('swap'); // USD1 to B
      expect(canonicalOrientation(USD1, SOL)).toBe('keep'); // USD1 already on B
    });
