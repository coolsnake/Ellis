 ...
 */
export async function scheduleDexApply(
  dex: DexSource,
  baseline: any
): Promise<void> {
  try {
 ...
 */
export function getApplyState(): Record<
  DexSource,
  { hasBaseline: boolean; hasTimer: boolean }
> {
  return {
    raydium: {
      hasBaseline: !!wsApply.raydium.baseline,
      hasTimer: !!wsApply.raydium.timer,
    },
    "raydium-cpmm": {
      hasBaseline: !!wsApply["raydium-cpmm"].baseline,
      hasTimer: !!wsApply["raydium-cpmm"].timer,
    },
    orca: {
      hasBaseline: !!wsApply.orca.baseline,
      hasTimer: !!wsApply.orca.timer,
    },
    meteora_dlmm: {
      hasBaseline: !!wsApply.meteora_dlmm.baseline,
      hasTimer: !!wsApply.meteora_dlmm.timer,
    },
    meteora_damm_v1: {
      hasBaseline: !!wsApply.meteora_damm_v1.baseline,
      hasTimer: !!wsApply.meteora_damm_v1.timer,
    },
    meteora_damm_v2: {
      hasBaseline: !!wsApply.meteora_damm_v2.baseline,
      hasTimer: !!wsApply.meteora_damm_v2.timer,
    },
    pumpswap: {
      hasBaseline: !!wsApply.pumpswap.baseline,
      hasTimer: !!wsApply.pumpswap.timer,
    },
  };
}
