...
export const METEORA_BALANCED_V1_PROGRAM =
  "Eo7WjKq67rjJQSZxS6z3YkapzY3eMj6Xy8X5EQVn5UaB";
export const METEORA_BALANCED_V2_PROGRAM =
...
async function loadDammV1Program(): Promise<any> {
  ...
  try {
    ...
    const { Program, AnchorProvider, Wallet } = await import(
      "@coral-xyz/anchor"
    );
    ...
    if (!AmmIdl) {
      logCatchDebug(
        "meteora_balanced.loadDammV1Program",
        "AmmIdl not found in dynamic-amm-sdk"
      );
 ...
 */
export async function decodeDammV1PoolAccount(
  data: Buffer
): Promise<DammV1PoolState | null> {
  try {
    ...
    if (tokenAMint === _SYS || tokenBMint === _SYS || tokenAMint === tokenBMint)
      return null;
...
async function loadDammV2Coder(): Promise<any> {
  ...
  try {
    ...
    logCatchDebug(
      "meteora_balanced.loadDammV2Coder",
      "cpAmmCoder not found in cp-amm-sdk"
    );
    dammV2CoderInitializing = false;
    return null;
  } catch (e) {
 ...
 */
export async function decodeDammV2PoolAccount(
  data: Buffer
): Promise<DammV2PoolState | null> {
  try {
    ...
    const tokenAMint = new PublicKey(
      buf.subarray(V2_OFFSET_TOKEN_A_MINT, V2_OFFSET_TOKEN_A_MINT + 32)
    ).toBase58();
    const tokenBMint = new PublicKey(
      buf.subarray(V2_OFFSET_TOKEN_B_MINT, V2_OFFSET_TOKEN_B_MINT + 32)
    ).toBase58();
    const tokenAVault = new PublicKey(
      buf.subarray(V2_OFFSET_TOKEN_A_VAULT, V2_OFFSET_TOKEN_A_VAULT + 32)
    ).toBase58();
    const tokenBVault = new PublicKey(
      buf.subarray(V2_OFFSET_TOKEN_B_VAULT, V2_OFFSET_TOKEN_B_VAULT + 32)
    ).toBase58();
    ...
    if (
      !tokenAMint ||
      !tokenBMint ||
      tokenAMint.length < 32 ||
      tokenBMint.length < 32 ||
      tokenAMint === _SYS2 ||
      tokenBMint === _SYS2 ||
      tokenAMint === tokenBMint
    ) {
      logCatchDebug(
        "meteora_balanced.decodeDammV2",
        "Invalid mints decoded from V2 pool"
      );
      return null;
    }
...
async function scheduleDexApply(
  source: "meteora_damm_v1" | "meteora_damm_v2",
      ...
      try {
        ...
        if (current && metbalApplyState.baseline) {
          ...
          if (typeof gmod?.applyPoolUpdates === "function") {
            void gmod
              .applyPoolUpdates(metbalApplyState.baseline, current, {
                pushToArb: false,
              })
              .catch((e: any) =>
                logCatchDebug("meteora_balanced.applyPoolUpdates", e)
              );
          }
        }
      } catch (e) {
...
export function decodeMeteoraBalancedPool(
  ...
  try {
    ...
    if (!mintA || !mintB) {
      logger.debug("meteora_balanced.decode.missing_mints", {
...
export async function handleMeteoraBalancedVaultUpdate(
  ...
  try {
    ...
    if (newBalance === null) {
      logger.debug("meteora_balanced.vault.parse.fail", {
    ...
    if (!poolData || poolData.source !== "meteora_balanced") {
      // Vault balance is already cached above in vaultBalanceCache — when the pool account
      // update arrives and creates the pool entry, it will read from vaultBalanceCache and
      // compute pricing immediately.  No explicit replay needed.
      logger.debug("meteora_balanced.vault.balance_cached_awaiting_pool", {
        vault: vaultAddress.slice(0, 8) + "…",
      ...
      return {
        success: true,
        skipped: true,
        skipReason: "pool_not_in_cache_balance_cached",
      };
    }
    ...
    const isV2Pool =
      dexName === "MeteoraBalanced_v2" || dexName.includes("_v2");
    ...
    if (isV2Pool && !storedSqrtPrice) {
      try {
        ...
        if (!account) {
          ...
          logger.warn(
            "meteora_balanced.vault.v2.sqrtPrice_fetch.small_buffer",
            {
              poolId: poolId.slice(0, 8) + "…",
          ...
          if (!decodedPool) {
            logger.warn(
              "meteora_balanced.vault.v2.sqrtPrice_fetch.decode_null",
              {
                poolId: poolId.slice(0, 8) + "…",
                dataLen: data.length,
                cat: "pools",
            ...
            decodedPool.sqrtPrice <= BigInt(0)
          ) {
            logger.warn(
              "meteora_balanced.vault.v2.sqrtPrice_fetch.sqrtPrice_zero",
              {
                poolId: poolId.slice(0, 8) + "…",
            ...
            if (storedLiquidity)
              (existingPool as any)._liquidity = storedLiquidity;
        ...
        logger.warn(
          "meteora_balanced.vault.v2.sqrtPrice_ondemand_fetch.error",
          {
            poolId: poolId.slice(0, 8) + "…",
    ...
    try {
      ...
      logCatchError("meteora_balanced.vault.cache", e);
    }
    ...
    wsDecodeStats.meteora_damm_v2.failures += 1;
    logCatchError("meteora_balanced.handleVaultUpdate", e, {
      vault: vaultAddress.slice(0, 8) + "…",
    });
...
export async function handleMeteoraBalancedPoolAccountUpdate(
  ...
  try {
    ...
    const minBufferLength = isV2
      ? MIN_DAMM_V2_POOL_BUFFER_LENGTH
      : MIN_DAMM_V1_POOL_BUFFER_LENGTH;
    ...
    if (!decoded) {
      ...
      return {
        success: false,
        error: "pool_state_decode_failed",
        skipped: true,
      };
    }
    ...
    if (decoded.aVault)
      vaultBalanceCache.set(`meta:${decoded.aVault}`, poolId as any);
    if (decoded.bVault)
      vaultBalanceCache.set(`meta:${decoded.bVault}`, poolId as any);
    ...
    const wasSwapped = (existingPool as any)?.was_swapped === true;
    let decA =
      existingPool?.native_decimals_a ??
      (wasSwapped ? existingPool?.decimals_b : existingPool?.decimals_a);
    let decB =
    ...
    const item: AmmPool = {
      ...
      ...(isV2 && decoded.sqrtPrice
        ? { _sqrtPrice: decoded.sqrtPrice.toString() }
        : {}),
      ...(isV2 && decoded.liquidity
        ? { _liquidity: decoded.liquidity.toString() }
        : {}),
    } as AmmPool;
    ...
    try {
      ...
      logCatchError("meteora_balanced.cache", e);
    }
    ...
    // Schedule graph update
    if (hasDelta) {
      await scheduleDexApply(
        isV2 ? "meteora_damm_v2" : "meteora_damm_v1",
        prev
      );
    }
    ...
    wsDecodeStats[statsKey].failures += 1;
    logCatchError("meteora_balanced.handlePoolAccountUpdate", e, {
      pool: poolId.slice(0, 8) + "…",
    });
    return { success: false, error: String((e as Error)?.message || e) };
  }
...
export async function handleMeteoraBalancedUpdate(
  ...
  try {
    ...
    return handleMeteoraBalancedVaultUpdate(
      info,
      accountAddress,
      derivedMeta.poolId
    );
    ...
    wsDecodeStats.meteora_damm_v2.failures += 1;
    logCatchError("meteora_balanced.handleUpdate", e, {
      account: accountAddress.slice(0, 8) + "…",
    });
    return { success: false, error: String((e as Error)?.message || e) };
  }
