...
const WHIRLPOOL_DISCRIMINATOR = Buffer.from([
...
async function loadWhirlpoolsClient(): Promise<typeof whirlpoolsClientModule> {
  ...
  try {
    ...
    logger.info("orca.decoder.new_client_loaded", {
      hasDecodeWhirlpool:
        typeof whirlpoolsClientModule.decodeWhirlpool === "function",
      hasGetDecoder:
        typeof whirlpoolsClientModule.getWhirlpoolDecoder === "function",
...
async function decodeWithNewClient(
  ...
  try {
    ...
    const mintA =
      typeof decoded.tokenMintA === "string"
        ? decoded.tokenMintA
        : decoded.tokenMintA?.toString?.() || "";
    const mintB =
      typeof decoded.tokenMintB === "string"
        ? decoded.tokenMintB
    ...
    const parsed = {
      ...
      tokenVaultA: decoded.tokenVaultA
        ? new PublicKey(decoded.tokenVaultA)
        : null,
      tokenVaultB: decoded.tokenVaultB
        ? new PublicKey(decoded.tokenVaultB)
        : null,
    ...
    if (sdk) {
      ...
      if (ParsableWhirlpool && typeof ParsableWhirlpool.parse === "function") {
        ...
        if (ownerPubkey) {
          ...
          try {
            const parsed = ParsableWhirlpool.parse(
              accountPubkey,
              sdkAccountInfo
            );
...
export async function handleOrcaUpdate(
  ...
  try {
    ...
    const decoded = await decodeOrcaWhirlpool(
      normalizedInfo,
      poolId,
      accountPubkey
    );
    ...
    const decA = cachedDec.decA ?? undefined;
    const decB = cachedDec.decB ?? undefined;

    if (!Number.isFinite(decA) || !Number.isFinite(decB)) {
      incrementSkipReason("orca", "decimals_pending");
      return {
        success: false,
    ...
    const canProcessPrice =
      Number.isFinite(decA) && Number.isFinite(decB) && sqrtRaw;
    ...
    const tick_spacing =
      Number.isFinite(rawTickSpacing) &&
      rawTickSpacing > 0 &&
      rawTickSpacing <= 512
        ? rawTickSpacing
        : 0;
        ...
        try {
          ...
          if (derived?.tickArrays) {
            ...
            needsTickArrayValidation =
              derived.needsTickArrayValidation || false;

            // Also update static cache with tick arrays for resolver access
            const existingStaticForArrays =
    ...
    if (hasDelta) {
      ...
      if (prevPool) {
        ...
        if (
          (prevPool as any).sqrt_price_x64_raw === clmmItem.sqrt_price_x64_raw
        )
          reasons.push("sqrt_price_unchanged");
        if ((prevPool as any).liquidity_raw === clmmItem.liquidity_raw)
          reasons.push("liquidity_raw_unchanged");
        if (
          Math.abs((prevPool.liquidity || 0) - (clmmItem.liquidity || 0)) === 0
        )
          reasons.push("liquidity_unchanged");
        if (
          Math.abs(
            (prevPool.price_a_per_b || 0) - (clmmItem.price_a_per_b || 0)
          ) <= 1e-9
        )
