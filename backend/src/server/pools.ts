...
const wsDecodeDebugCounters: Record<'raydium' | 'orca' | 'meteora', number> = { raydium: 0, orca: 0, meteora: 0 };
const wsDecodeSkipDebugCounters: Record<'raydium' | 'orca' | 'meteora', number> = { raydium: 0, orca: 0, meteora: 0 };
...
function debugLogDecode(source: 'raydium' | 'orca' | 'meteora', account: string, extra: Record<string, unknown>): void {
  try {
    const limit = Number((CONFIG.system as any)?.wsDebugDecodeLogLimit ?? (CONFIG.system as any)?.wsDebugAccountLogLimit ?? 10);
    if (!(limit > 0)) return;
    if (wsDecodeDebugCounters[source] >= limit) return;
    wsDecodeDebugCounters[source] += 1;
    logger.info('pools.ws debug.decode', { source, account, ...extra, cat: 'pools' });
  } catch {}
}

function debugLogDecodeSkip(source: 'raydium' | 'orca' | 'meteora', account: string, reason: string, extra: Record<string, unknown> = {}): void {
  try {
    const limit = Number((CONFIG.system as any)?.wsDebugDecodeLogLimit ?? (CONFIG.system as any)?.wsDebugAccountLogLimit ?? 10);
    if (!(limit > 0)) return;
    if (wsDecodeSkipDebugCounters[source] >= limit) return;
    wsDecodeSkipDebugCounters[source] += 1;
    logger.info('pools.ws debug.decode_skip', { source, account, reason, ...extra, cat: 'pools' });
  } catch {}
}
              ...
              try {
                ...
                if (!rmod || !info?.data) { throw new Error('raydium sdk missing'); }
                  ...
                  if (clmmLayout && typeof clmmLayout.decode === 'function') {
                    ...
                    const mintAState = (state as any)?.mintA || (state as any)?.tokenMintA;
                    const mintBState = (state as any)?.mintB || (state as any)?.tokenMintB;
                    const hasLiquidity = state && (state as any).liquidity != null;
                    if (state && hasLiquidity && mintAState && mintBState) {
                      const mintA = mintAState?.toBase58?.() || mintAState?.toString?.() || '';
                      const mintB = mintBState?.toBase58?.() || mintBState?.toString?.() || '';
                  ...
                  try {
                    ...
                    if (inc && hasDelta) {
                      ...
                      debugLogDecodeSkip('raydium', pk58, 'clmm_missing_fields', {
                        hasLiquidity,
                        hasMintA: !!mintAState,
                        hasMintB: !!mintBState,
                      });
                    } else {
                      debugLogDecodeSkip('raydium', pk58, 'clmm_layout_decode_empty', { dataLen: Number(info?.data?.length ?? 0) });
                    }
                  }
                  // Try AMM V4 decode
                  if (!updated) {
                    ...
                    if (ammLayout && typeof ammLayout.decode === 'function') {
                      ...
                      if (state) {
                        ...
                        debugLogDecodeSkip('raydium', pk58, 'amm_layout_decode_empty', { dataLen: Number(info?.data?.length ?? 0) });
                      }
                    }
                  } catch (e:any) {
