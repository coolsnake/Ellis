...
import {
  fetchRaydiumPoolsRaw as fetchRaydiumPoolsRawImpl,
  normalizeRaydiumPools as normalizeRaydiumPoolsImpl,
} from "./pools/raydium.js";
import {
  fetchOrcaHttp as fetchOrcaHttpImpl,
  normalizeOrcaHttp as normalizeOrcaHttpImpl,
} from "./pools/orca.js";
import {
  fetchMeteoraHttp as fetchMeteoraHttpImpl,
  normalizeMeteoraHttp as normalizeMeteoraHttpImpl,
} from "./pools/meteora.js";
import {
  fetchPumpswapGraphQL as fetchPumpswapGraphQLImpl,
  normalizePumpswapPools as normalizePumpswapPoolsImpl,
  enrichPumpswapPoolsWithRpc as enrichPumpswapPoolsWithRpcImpl,
  fetchPumpswapSummaryOnly,
} from "./pools/pumpswap.js";
// Early filter optimization imports
import {
  fetchRaydiumSummaryOnly,
  fetchRaydiumClmmSummaryOnly,
  fetchRaydiumPoolsByAddress,
  fetchRaydiumClmmPoolsByAddress,
  normalizeRaydiumGraphQL,
} from "./pools/raydiumGraphQL.js";
import {
  fetchOrcaSummaryOnly,
  fetchOrcaPoolsByAddress,
  normalizeOrcaGraphQL,
} from "./pools/orcaGraphQL.js";
import {
  fetchMeteoraSummaryOnly,
...
import {
  loadPersistedDecimals,
  flushPersistedDecimals,
} from "./pools/decimals.js";
...
import {
  ...
  httpLog429,
  httpLogNonOk,
} from "./pools/httpLog.js";
import {
  fetchMeteoraBalancedHttp as fetchMeteoraBalancedHttpImpl,
  normalizeMeteoraBalancedHttp as normalizeMeteoraBalancedHttpImpl,
  fetchMeteoraBalancedAll as fetchMeteoraBalancedAllImpl,
} from "./pools/meteoraBalanced.js";
...
import {
  ...
  deriveTickArrayPda,
} from "../execution/raydiumTickArrays.js";
...
export async function normalizeRaydiumPools(raw: any): Promise<PoolsPayload> {
  return normalizeRaydiumPoolsImpl(raw);
}
...
export async function defaultNormalizeRaydiumPools(
  raw: any
  ...
  try {
    ...
    enrichedDecimals = await enrichPoolTokenDecimals(arr, {
      logger,
      forceOnchain: true,
    });
  } catch (err: any) {
    try {
      logger.warn("raydium.normalizer.enrich.failed", {
  ...
  for (const it of arr || []) {
    ...
    const pooltype = Array.isArray((it as any)?.pooltype)
      ? (it as any).pooltype
      : [];
    const tickSpacing =
      (it as any)?.tickSpacing ?? (it as any)?.config?.tickSpacing;
    ...
    const isClmm =
      ...
      hasTick ||
      typeof (it as any)?.sqrtPriceX64 !== "undefined" ||
      typeof (it as any)?.sqrtPrice !== "undefined";
    const fee_bps = toFeeBps(
      (it as any)?.feeRate ??
        (it as any)?.tradeFeeRate ??
        (it as any)?.feeBps ??
        (it as any)?.tradeFeeBps
    );
    ...
    if (typeof enrichedA === "number" && Number.isFinite(enrichedA))
      decA = enrichedA;
    if (typeof enrichedB === "number" && Number.isFinite(enrichedB))
      decB = enrichedB;
    ...
    if (isClmm) {
      const tick = Number(
        (it as any)?.tickSpacing ?? (it as any)?.config?.tickSpacing ?? 0
      );
      const sqrtCandidate =
        (it as any)?.sqrtPriceX64 ?? (it as any)?.sqrtPrice ?? 0;
      const sqrtBig = anyToBigInt(sqrtCandidate);
      const sqrt =
        typeof sqrtCandidate === "number"
          ? sqrtCandidate
          : Number(sqrtBig ?? 0n);
      ...
      if (sqrtBig && Number.isFinite(decA) && Number.isFinite(decB)) {
        ratio = sqrtPriceX64ToPriceRatio(
          sqrtBig,
          decA as number,
          decB as number
        );
      ...
      const amount_a_whole = Number.isFinite(mintAmountA)
        ? mintAmountA
        : Number.isFinite(reserveA)
        ? reserveA
        : undefined;
      const amount_b_whole = Number.isFinite(mintAmountB)
        ? mintAmountB
      ...
      const price_res =
        Number.isFinite(amount_a_whole as any) &&
        Number.isFinite(amount_b_whole as any) &&
        (amount_a_whole as number) > 0
          ? (amount_b_whole as number) / (amount_a_whole as number)
          : reserveA0 > 0
          ? reserveB0 / reserveA0
          : 0;
      const price_res_decs =
        Number.isFinite(decA) &&
        Number.isFinite(decB) &&
        Number.isFinite(mintAmountA) &&
        Number.isFinite(mintAmountB) &&
        (mintAmountA as number) > 0
          ? (mintAmountB as number) /
            Math.pow(10, decB as number) /
            ((mintAmountA as number) / Math.pow(10, decA as number))
          : 0;
      const px =
        price_res_decs > 0
          ? price_res_decs
...
export async function refreshAllSources(
  ...
  const phaseStats: {
    fetch?: {
      raydium: { amm: number; clmm: number; cpmm?: number };
      orca: { amm: number; clmm: number };
      meteora: { amm: number; clmm: number };
      meteora_balanced: { amm: number; clmm: number };
      pumpswap: { amm: number; clmm: number };
    };
    universe?: {
      mode: string;
    ...
    tvl?: {
      minAmm: number;
      minClmm: number;
      before: Record<string, { a: number; c: number }>;
      after: Record<string, { a: number; c: number }>;
    };
    activity?: {
  ...
  if ((refreshAllSources as any).__isRunning) {
    try {
      logger.info("pools.refresh.skipped", {
  ...
  try {
    ...
    if (loadedCount > 0) {
      logger.info("pools.refresh.decimals.loaded", {
        count: loadedCount,
        cat: "decimals",
      });
    }
  } catch (e: any) {
    logger.warn("pools.refresh.decimals.load_failed", {
  ...
  const shouldFetch = {
    ...
    meteora_balanced:
      options.sources?.meteora_balanced ?? meteoraBalancedEnabled,
  ...
  try {
    ...
    if (force) {
      ...
      const pauseFeed =
        (CONFIG.system as any)?.pausePriceFeedDuringBootstrap !== false;
      if (pauseFeed) {
        try {
          emit("log", {
            level: "info",
            message: "pools:bootstrap api.pause",
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        } catch {}
        try {
          logger.info("pools:bootstrap api.pause", { cat: "pools" });
        } catch {}
        try {
        ...
        try {
          (await import("../jupiter/rateLimiter.js")).apiStop();
        } catch {}
      }
      ...
      if (pauseFeed) {
        try {
          (await import("../jupiter/rateLimiter.js")).apiStart();
        } catch {}
        try {
          emit("log", {
            level: "info",
            message: "pools:bootstrap api.resume",
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        } catch {}
        try {
          logger.info("pools:bootstrap api.resume", { cat: "pools" });
        } catch {}
  ...
  try {
    ...
    const tokenUniMode = String(
      (CONFIG.system as any)?.tokenUniverseMode || "union"
    );
    if (tokenUniMode === "jupiterTop") warmModes.add("jupiterTop");
    const scopeMode = String((CONFIG.system as any)?.scopePoolsMode || "none");
    const scopingEnabled =
      (CONFIG.system as any)?.scopePools !== false && scopeMode !== "none";
    if (scopingEnabled && scopeMode === "jupiterTop")
      warmModes.add("jupiterTop");
    if (warmModes.size > 0) {
      ...
      for (const mode of warmModes) {
        try {
          ...
          logger.warn("pools.refresh.universe_prewarm_failed", {
            mode,
            error: String(err?.message || err),
            cat: "pools",
          });
      ...
      logger.info("pools.refresh.universe_prewarm.complete", {
        modes: Array.from(warmModes),
        cat: "pools",
      });
    }
  } catch (err: any) {
    logger.warn("pools.refresh.universe_prewarm.error", {
      error: String(err?.message || err),
      cat: "pools",
    });
  }
  ...
  const enableEarlyFilter =
    (CONFIG.system as any)?.enableEarlyMinPoolsFilter !== false;
  ...
  const minPoolsThreshold = Math.max(
    1,
    Number((CONFIG.system as any)?.minPoolsPerPair || 1)
  );

  // Early filter optimization applies when: enabled in config AND minPools > 1 AND GraphQL is used
  const useEarlyFilterPath =
    enableEarlyFilter &&
    minPoolsThreshold > 1 &&
  ...
  logger.info("pools.refresh.phase.fetch", {
    ...
    minPoolsThreshold,
    graphql: {
      raydium: useRaydiumGraphQL,
      orca: useOrcaGraphQL,
      meteora: useMeteoraGraphQL,
    },
    cat: "pools",
  });
  ...
  if (useEarlyFilterPath) {
    ...
    logger.info("pools.refresh.phase.fetch.early_filter_path", {
      cat: "pools",
    });

    // Get token universe for summary queries - compute once and share with all fetchers
    const { computeTokenUniverse, getAnchorSet } = await import(
      "./universe.js"
    );
    const universe = await computeTokenUniverse(
      (CONFIG.system as any)?.tokenUniverseMode
    );
    ...
    logger.info("pools.refresh.phase.1A.summary_fetch", {
    ...
    if (shouldFetch.raydium && useRaydiumGraphQL) {
      try {
        ...
        if (isAmmEnabled) {
          ...
          logger.debug("pools.refresh.phase.1A.raydium.amm", {
            count: raySummary.length,
            cat: "pools",
          });
        } else {
          logger.debug("pools.refresh.phase.1A.raydium.amm.skipped", {
            reason: "disabled",
            cat: "pools",
          });
        }
        ...
        if (isClmmEnabled) {
          ...
          if (isAmmEnabled) {
            const clmmPageDelayMs = Number(
              (CONFIG as any)?.raydiumClmm?.pageDelayMs || 200
            );
            const interPhaseMultiplier = Number(
              (CONFIG as any)?.raydiumClmm?.initialDelayMultiplier || 10
            );
            await new Promise((resolve) =>
              setTimeout(resolve, clmmPageDelayMs * interPhaseMultiplier)
            );
          }

          rayClmmSummary = await fetchRaydiumClmmSummaryOnly(mints);
          logger.debug("pools.refresh.phase.1A.raydium.clmm", {
            count: rayClmmSummary.length,
            cat: "pools",
          });
        } else {
          logger.debug("pools.refresh.phase.1A.raydium.clmm.skipped", {
            reason: "disabled",
            cat: "pools",
          });
        }
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1A.raydium.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (shouldFetch.orca && useOrcaGraphQL) {
      try {
        ...
        logger.debug("pools.refresh.phase.1A.orca", {
          count: orcaSummary.length,
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1A.orca.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (shouldFetch.meteora && useMeteoraGraphQL) {
      try {
        ...
        logger.debug("pools.refresh.phase.1A.meteora", {
          count: metSummary.length,
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1A.meteora.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (shouldFetch.pumpswap) {
      try {
        ...
        logger.debug("pools.refresh.phase.1A.pumpswap", {
          count: pumpSummary.length,
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1A.pumpswap.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    const summaryCounts = {
      ...
      total:
        raySummary.length +
        rayClmmSummary.length +
        orcaSummary.length +
        metSummary.length +
        pumpSummary.length,
    };
    logger.info("pools.refresh.phase.1A.complete", {
      counts: summaryCounts,
      cat: "pools",
    });

    // === PHASE 1B: EARLY UNIVERSE FILTER (on summaries) ===
    logger.info("pools.refresh.phase.1B.early_universe_filter", {
      cat: "pools",
    });
    const scopeMode = String(
      (CONFIG.system as any)?.scopePoolsMode || "jupiter"
    );
    const scoped = CONFIG.system.scopePools !== false && scopeMode !== "none";
    const anchorBridging = !!(CONFIG.system as any)?.enableAnchorBridging;

    const filterSummariesByUniverse = (
      summaries: SummaryPool[],
      uni: Set<string>,
      anchorBridge: boolean
    ): SummaryPool[] => {
    ...
    if (scoped) {
      ...
      raySummary = filterSummariesByUniverse(
        raySummary,
        universe,
        anchorBridging
      );
      rayClmmSummary = filterSummariesByUniverse(
        rayClmmSummary,
        universe,
        anchorBridging
      );
      orcaSummary = filterSummariesByUniverse(
        orcaSummary,
        universe,
        anchorBridging
      );
      metSummary = filterSummariesByUniverse(
        metSummary,
        universe,
        anchorBridging
      );
      pumpSummary = filterSummariesByUniverse(
        pumpSummary,
        universe,
        anchorBridging
      );
      ...
      logger.info("pools.refresh.phase.1B.complete", {
        mode: scopeMode,
        before: beforeCounts,
        after: afterCounts,
        cat: "pools",
    ...
    logger.info("pools.refresh.phase.1C.early_min_pools_filter", {
    ...
    const totalBefore = Object.values(beforeMinPools).reduce(
      (a, b) => a + b,
      0
    );
    ...
    logger.info("pools.refresh.phase.1D.detail_fetch", {
      survivors: afterMinPools,
      cat: "pools",
    });

    // Fetch details only for survivor pools
    const detailBatchSize = Number(
      (CONFIG as any)?.raydium?.detailBatchSize || 50
    );
    const detailDelayMs = Number(
      (CONFIG as any)?.raydium?.detailBatchDelayMs || 200
    );
    ...
    if (survivorIds.raydiumAmm.size > 0) {
      try {
        const rayDetailMap = await fetchRaydiumPoolsByAddress(
          Array.from(survivorIds.raydiumAmm),
          {
            retries,
            backoffMs,
        ...
        const rayNormalized = await normalizeRaydiumGraphQL([
          ...rayDetailMap.values(),
        ]);
        r.amm = rayNormalized.amm || [];
        logger.debug("pools.refresh.phase.1D.raydium.amm", {
          count: r.amm.length,
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1D.raydium.amm.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (survivorIds.raydiumClmm.size > 0) {
      try {
        ...
        const rayClmmDetailMap = await fetchRaydiumClmmPoolsByAddress(
          Array.from(survivorIds.raydiumClmm),
          {
            retries,
            backoffMs,
            batchSize: detailBatchSize,
        ...
        const rayClmmNormalized = await normalizeRaydiumGraphQL([
          ...rayClmmDetailMap.values(),
        ]);
        r.clmm = rayClmmNormalized.clmm || [];
        logger.debug("pools.refresh.phase.1D.raydium.clmm", {
          count: r.clmm.length,
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1D.raydium.clmm.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (survivorIds.orca.size > 0) {
      try {
        ...
        const orcaDetailMap = await fetchOrcaPoolsByAddress(
          Array.from(survivorIds.orca),
          {
        ...
        logger.debug("pools.refresh.phase.1D.orca", {
          count: (o.amm?.length || 0) + (o.clmm?.length || 0),
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1D.orca.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (survivorIds.meteora.size > 0) {
      try {
        ...
        const metDetailMap = await fetchMeteoraPoolsByAddress(
          Array.from(survivorIds.meteora),
          {
            ...
            delayMs: Number(
              (CONFIG as any)?.meteora?.detailBatchDelayMs || 200
            ),
          }
        );
        m = await normalizeMeteoraGraphQL([...metDetailMap.values()]);
        logger.debug("pools.refresh.phase.1D.meteora", {
          count: (m.amm?.length || 0) + (m.clmm?.length || 0),
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1D.meteora.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (survivorIds.pumpswap.size > 0 && pumpRawPools.length > 0) {
      try {
        const survivorRaw = pumpRawPools.filter((p) =>
          survivorIds.pumpswap.has(p.pubkey)
        );
        ...
        const { pools: enriched } = await enrichPumpswapPoolsWithRpcImpl(
          survivorRaw
        );
        pump = await normalizePumpswapPoolsImpl(enriched);
        logger.debug("pools.refresh.phase.1D.pumpswap", {
          count: pump.amm?.length || 0,
          cat: "pools",
        });
      } catch (e: any) {
        logger.warn("pools.refresh.phase.1D.pumpswap.failed", {
          error: String(e?.message || e),
          cat: "pools",
        });
      }
    ...
    if (shouldFetch.raydium) {
      try {
        ...
        const isCpmmEnabledPreCheck =
          (CONFIG as any)?.raydiumCpmm?.enabled !== false;
        ...
        if (isAmmEnabled) {
          r = useGraphQL
            ...
            : await getRaydiumPoolsNormalized(!!options.force, {
                skipUniverseFilter: true,
              });
        } else {
          logger.info("pools.refresh.phase.fetch.raydium.amm.skipped", {
            reason: "disabled",
            cat: "pools",
          });
        }
        ...
        if (useGraphQL && isClmmEnabled) {
          try {
            ...
            if (isAmmEnabled) {
              ...
              if (interPhaseDelayMs > 0) {
                await new Promise((resolve) =>
                  setTimeout(resolve, interPhaseDelayMs)
                );
              }
            }

            const clmmResult = await getRaydiumClmmPoolsGraphQL(
              !!options.force,
              { mints: sharedMints }
            );
          ...
          logger.info("pools.refresh.phase.fetch.raydium.clmm.skipped", {
            reason: "disabled",
            cat: "pools",
          });
        }
        ...
        if (isCpmmEnabled && (CONFIG as any)?.raydiumCpmm?.enabled !== false) {
          try {
            ...
            if (isAmmEnabled || isClmmEnabled) {
              const cpmmPageDelayMs = Number(
                (CONFIG as any)?.raydiumCpmm?.pageDelayMs || 200
              );
              const interPhaseMultiplier = Number(
                (CONFIG as any)?.raydiumCpmm?.initialDelayMultiplier || 10
              );
            ...
            const cpmmResult = await getRaydiumCpmmPoolsGraphQL(
              !!options.force,
              { mints: sharedMints }
            );
          ...
          logger.info("pools.refresh.phase.fetch.raydium.cpmm.skipped", {
            reason: "disabled",
            cat: "pools",
          });
        }
      } catch (err) {
        logger.warn("pools.refresh.phase.fetch.raydium.failed", {
          error: String((err as any)?.message || err),
          cat: "pools",
        });
        r = { amm: [], clmm: [], cpmm: [] };
      ...
      logger.info("pools.refresh.phase.fetch.raydium.skipped", {
        reason: "disabled",
        cat: "pools",
      });
    }
    ...
    if (shouldFetch.orca) {
      try {
        o = (CONFIG as any)?.orca?.useGraphQL
          ...
          : await getOrcaPoolsCached(!!options.force, {
              skipUniverseFilter: true,
            });
        ...
        logger.warn("pools.refresh.phase.fetch.orca.failed", {
          error: String((err as any)?.message || err),
          cat: "pools",
        });
        o = { amm: [], clmm: [], cpmm: [] };
      ...
      logger.info("pools.refresh.phase.fetch.orca.skipped", {
        reason: "disabled",
        cat: "pools",
      });
    }
    ...
    if (shouldFetch.meteora) {
      try {
        m = (CONFIG as any)?.meteora?.useGraphQL
          ...
          : await getMeteoraPoolsCached(!!options.force, {
              skipUniverseFilter: true,
            });
      } catch (err) {
        logger.warn("pools.refresh.phase.fetch.meteora.failed", {
          error: String((err as any)?.message || err),
          cat: "pools",
        });
        m = { amm: [], clmm: [], cpmm: [] };
      ...
      logger.info("pools.refresh.phase.fetch.meteora.skipped", {
        reason: "disabled",
        cat: "pools",
      });
    }
  ...
  if (shouldFetch.meteora_balanced) {
    try {
      ...
      logger.warn("pools.refresh.phase.fetch.meteora_balanced.failed", {
        error: String((err as any)?.message || err),
        cat: "pools",
      });
      mb = { amm: [], clmm: [], cpmm: [] };
    ...
    logger.info("pools.refresh.phase.fetch.meteora_balanced.skipped", {
      reason: "disabled",
      cat: "pools",
    });
  }
  ...
  if (!useEarlyFilterPath && shouldFetch.pumpswap) {
    try {
      ...
      logger.warn("pools.refresh.phase.fetch.pumpswap.failed", {
        error: String((err as any)?.message || err),
        cat: "pools",
      });
      pump = { amm: [], clmm: [], cpmm: [] };
    ...
    logger.info("pools.refresh.phase.fetch.pumpswap.skipped", {
      reason: "disabled",
      cat: "pools",
    });
  }
  ...
  try {
    const fetchCounts = {
      raydium: {
        amm: r.amm?.length || 0,
        clmm: r.clmm?.length || 0,
        cpmm: r.cpmm?.length || 0,
      },
      orca: { amm: o.amm?.length || 0, clmm: o.clmm?.length || 0 },
      meteora: { amm: m.amm?.length || 0, clmm: m.clmm?.length || 0 },
      meteora_balanced: {
        amm: mb.amm?.length || 0,
        clmm: mb.clmm?.length || 0,
      },
      pumpswap: { amm: pump.amm?.length || 0, clmm: pump.clmm?.length || 0 },
    };
    phaseStats.fetch = fetchCounts;
    logger.info("pools.refresh.phase.fetch.complete", {
      counts: fetchCounts,
      earlyFiltersApplied,
      cat: "pools",
    });
  ...
  if (earlyFiltersApplied) {
    logger.info("pools.refresh.phase.universe_filter.skipped", {
      reason: "early_filters_applied",
      cat: "pools",
    });
  } else {
  ...
  try {
    ...
    if (scoped && !earlyFiltersApplied) {
      const { computeTokenUniverse, filterPoolsByUniverse } = await import(
        "./universe.js"
      );
      ...
      const mbScoped = filterPoolsByUniverse(
        mb as any,
        universe,
        anchorBridging
      );
      const pScoped = filterPoolsByUniverse(
        pump as any,
        universe,
        anchorBridging
      );
      ...
      const beforeCounts = {
        raydium:
          (r.amm?.length || 0) + (r.clmm?.length || 0) + (r.cpmm?.length || 0),
    ...
    logger.warn("pools.refresh.phase.universe_filter.failed", {
      error: String(e?.message || e),
      cat: "pools",
    });
  }
  ...
  try {
    ...
    if (minPools > 1 && !earlyFiltersApplied) {
      ...
      const filt = <T extends { mint_a: string; mint_b: string }>(arr: T[]) =>
        (arr || []).filter((p) =>
          allow.has(canonicalPairKey(p.mint_a, p.mint_b))
        );
    ...
    logger.warn("pools.refresh.phase.min_pools_filter.failed", {
      error: String(e?.message || e),
      cat: "pools",
    });
  }
  ...
  logger.info("pools.refresh.phase.enrich", {
    note: "enrichment_embedded_in_normalizers",
    cat: "pools",
  });
  ...
  try {
    const minAmm = Math.max(
      0,
      Number((CONFIG.system as any)?.minAmmLiqBase ?? 0)
    );
    const minClmm = Math.max(
      0,
      Number((CONFIG.system as any)?.minClmmLiquidity ?? 0)
    );
    if (minAmm > 0 || minClmm > 0) {
      ...
      const filt = (norm: { amm: any[]; clmm: any[]; cpmm?: any[] }) => ({
        amm:
          minAmm > 0
            ? (norm.amm || []).filter((p: any) => valAmm(p) >= minAmm)
            : norm.amm || [],
        clmm:
          minClmm > 0
            ? (norm.clmm || []).filter((p: any) => valClmm(p) >= minClmm)
            : norm.clmm || [],
        cpmm:
          minAmm > 0
            ? (norm.cpmm || []).filter((p: any) => valAmm(p) >= minAmm)
            : norm.cpmm || [],
      });
    ...
    logger.warn("pools.refresh.phase.tvl_filter.failed", {
      error: String(e?.message || e),
      cat: "pools",
    });
  }
  ...
  try {
    const maxInactiveMs = Number(
      (CONFIG.system as any)?.maxInactivePoolMs ?? 12 * 60 * 60 * 1000
    ); // Default 12 hours
    const enableActivityFilter =
      maxInactiveMs > 0 &&
      (CONFIG.system as any)?.enableActivityFilter !== false;

    if (enableActivityFilter) {
      // Collect all pool IDs from all sources
      const allPoolIds: string[] = [];
      const poolSourceMap = new Map<
        string,
        { source: string; type: "amm" | "clmm" }
      >();
      ...
      if (allPoolIds.length > 0) {
        ...
        const totalBefore = Object.values(beforeCounts).reduce(
          (sum, v) => sum + v.a + v.c,
          0
        );
        const totalAfter = Object.values(afterCounts).reduce(
          (sum, v) => sum + v.a + v.c,
          0
        );
        phaseStats.activity = {
        ...
        logger.info("pools.refresh.phase.activity_filter.complete", {
          ...
          removed: {
            ...
            meteora_balanced: {
              a:
                beforeCounts.meteora_balanced.a -
                afterCounts.meteora_balanced.a,
              c:
                beforeCounts.meteora_balanced.c -
                afterCounts.meteora_balanced.c,
            },
            pumpswap: {
  ...
  try {
    ...
    logger.warn("pools.refresh.phase.min_pools_filter_2nd.failed", {
      error: String(e?.message || e),
      cat: "pools",
    });
  }
  ...
  // Rebuild O(1) pool lookup index after bulk cache replacement
  try {
    const { rebuildPoolIndex } = await import("./pools.cache.js");
    rebuildPoolIndex();
  } catch {}
  ...
  try {
    if (force) {
      ...
      const addFrom = (pp: PoolsPayload) => {
        try {
          for (const p of pp?.amm || []) {
            if (p?.mint_a) mintSet.add(String(p.mint_a));
            if (p?.mint_b) mintSet.add(String(p.mint_b));
          }
        } catch {}
        try {
          for (const p of pp?.clmm || []) {
            if (p?.mint_a) mintSet.add(String(p.mint_a));
            if (p?.mint_b) mintSet.add(String(p.mint_b));
          }
        } catch {}
      ...
      if (mintSet.size > 0) {
        ...
        let pricedCount = 0;
        for (const x of mintSet) {
          if (typeof (pricedMap as any)[x]?.usdc === "number") pricedCount++;
        }
        const coverage = pricedCount / Math.max(1, mintSet.size);
        const minPriced = Math.max(
          50,
          Number((CONFIG.system as any)?.minMintPriceBootstrap || 120)
        );
        const minCoverage = Math.max(
          0.05,
          Math.min(
            0.9,
            Number((CONFIG.system as any)?.minMintPriceCoverage || 0.4)
          )
        );
  ...
  try {
    const { validateCrossDexPrices, filterAnomalousPrices } = await import(
      "./pools/validation.js"
    );
    const { validateCrossDexPricesSimple } = await import(
      "./pools/comprehensiveValidation.js"
    );
    const allPools = {
      raydium: r,
      orca: o,
    ...
    const loggingThreshold = Number(
      (CONFIG.system as any)?.crossDexLoggingThreshold || 0.05
    ); // 5%
    validateCrossDexPrices(allPools, loggingThreshold);

    // Simplified validation (no comprehensive checks)
    validateCrossDexPricesSimple(allPools, 0.1);

    // Filter out severe anomalies
    const filteringThreshold = Number(
      (CONFIG.system as any)?.crossDexFilteringThreshold || 0.1
    ); // 10%
    ...
    logger.warn("pools.refresh.phase.validation.failed", {
      error: String(e?.message || e),
      cat: "pools",
    });
  }
  ...
  try {
    ...
    const compute = (
      p: any
    ): { forward: number | undefined; reverse: number | undefined } => {
      try {
    ...
    if (rayPick) {
      ...
      if (forward && reverse) {
        try {
          logger.debug("pools.pair_sol_usdc", {
            source: "raydium",
            id: rayPick.id,
            kind:
              rayPick.pool_kind ||
              (rayPick.sqrt_price_x64 != null ? "clmm" : "amm"),
            forward_usdc_per_sol: forward,
            reverse_sol_per_usdc: reverse,
            cat: "pools",
          });
        } catch {}
        try {
          emit("log", {
            level: "info",
            message: `pools:pair_sol_usdc source=raydium id=${
              rayPick.id
            } kind=${
              rayPick.pool_kind ||
              (rayPick.sqrt_price_x64 != null ? "clmm" : "amm")
        ...
        try {
          logger.debug("pools.pair_sol_usdc.skip", {
            source: "raydium",
            reason: "invalid_price_or_orientation",
            id: rayPick.id,
            a: rayPick.mint_a,
            b: rayPick.mint_b,
            px: rayPick.price_a_per_b,
            cat: "pools",
          });
        } catch {}
        try {
          emit("log", {
            level: "debug",
            message: `pools:pair_sol_usdc.skip source=raydium reason=invalid_price_or_orientation id=${rayPick.id} a=${rayPick.mint_a} b=${rayPick.mint_b} px=${rayPick.price_a_per_b}`,
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        } catch {}
      ...
      try {
        emit("log", {
          level: "debug",
          message: "pools:pair_sol_usdc.skip source=raydium reason=no_sol_usdc",
          timestamp: new Date().toISOString(),
          context: { cat: "pools" },
        });
      } catch {}
    ...
    if (orcPick) {
      ...
      if (forward && reverse) {
        try {
          logger.debug("pools.pair_sol_usdc", {
            ...
            kind:
              orcPick.pool_kind ||
              (orcPick.sqrt_price_x64 != null ? "clmm" : "amm"),
            forward_usdc_per_sol: forward,
            reverse_sol_per_usdc: reverse,
            cat: "pools",
          });
        ...
        try {
          emit("log", {
            level: "info",
            message: `pools:pair_sol_usdc source=orca id=${orcPick.id} kind=${
              orcPick.pool_kind ||
              (orcPick.sqrt_price_x64 != null ? "clmm" : "amm")
            } fwd=${forward} rev=${reverse}`,
            timestamp: new Date().toISOString(),
        ...
        try {
          emit("log", {
            level: "debug",
            message: `pools:pair_sol_usdc.skip source=orca reason=invalid_price_or_orientation id=${orcPick.id} a=${orcPick.mint_a} b=${orcPick.mint_b} px=${orcPick.price_a_per_b}`,
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        } catch {}
      ...
      try {
        emit("log", {
          level: "debug",
          message: "pools:pair_sol_usdc.skip source=orca reason=no_sol_usdc",
          timestamp: new Date().toISOString(),
          context: { cat: "pools" },
        });
      } catch {}
    ...
    if (metPick) {
      ...
      if (forward && reverse) {
        try {
          logger.debug("pools.pair_sol_usdc", {
            ...
            kind:
              metPick.pool_kind ||
              (metPick.sqrt_price_x64 != null ? "clmm" : "amm"),
            forward_usdc_per_sol: forward,
            reverse_sol_per_usdc: reverse,
            cat: "pools",
          });
        ...
        try {
          emit("log", {
            level: "info",
            message: `pools:pair_sol_usdc source=meteora id=${
              metPick.id
            } kind=${
              metPick.pool_kind ||
              (metPick.sqrt_price_x64 != null ? "clmm" : "amm")
        ...
        try {
          emit("log", {
            level: "debug",
            message: `pools:pair_sol_usdc.skip source=meteora reason=invalid_price_or_orientation id=${metPick.id} a=${metPick.mint_a} b=${metPick.mint_b} px=${metPick.price_a_per_b}`,
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        } catch {}
      ...
      try {
        emit("log", {
          level: "debug",
          message: "pools:pair_sol_usdc.skip source=meteora reason=no_sol_usdc",
          timestamp: new Date().toISOString(),
          context: { cat: "pools" },
        });
      } catch {}
    ...
    if (pumpPick) {
      ...
      if (forward && reverse) {
        ...
        try {
          emit("log", {
            level: "info",
            message: `pools:pair_sol_usdc source=pumpswap id=${
              pumpPick.id
            } kind=${
              pumpPick.pool_kind || "amm"
            } fwd=${forward} rev=${reverse}`,
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        ...
        try {
          emit("log", {
            level: "debug",
            message: `pools:pair_sol_usdc.skip source=pumpswap reason=invalid_price_or_orientation id=${pumpPick.id} a=${pumpPick.mint_a} b=${pumpPick.mint_b} px=${pumpPick.price_a_per_b}`,
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        } catch {}
      ...
      try {
        emit("log", {
          level: "debug",
          message:
            "pools:pair_sol_usdc.skip source=pumpswap reason=no_sol_usdc",
          timestamp: new Date().toISOString(),
          context: { cat: "pools" },
        });
      } catch {}
    ...
    try {
      logger.warn("pools.pair_sol_usdc.failed", {
        error: String(e?.message || e),
        cat: "pools",
      });
    } catch {}
    try {
      emit("log", {
        level: "warn",
        message: `pools:pair_sol_usdc.failed error=${String(e?.message || e)}`,
        timestamp: new Date().toISOString(),
        context: { cat: "pools" },
      });
    } catch {}
  ...
  try {
    if (force) {
      const reg: any = await import("./feedRegistry.js");
      const wantResume =
        (refreshAllSources as any).__resumeFeed === true ||
        reg.isPriceFeedEnabled?.() === true;
      ...
      try {
        emit("log", {
          level: "info",
          message: `pools:bootstrap feed.resume enabled=${shouldEnable}`,
          timestamp: new Date().toISOString(),
          context: { cat: "pools" },
        });
      } catch {}
      try {
        logger.info(`pools:bootstrap feed.resume enabled=${shouldEnable}`, {
          cat: "pools",
        });
      } catch {}
    }
  ...
  try {
    const { shouldAutoStartSubscriptions } = await import(
      "./pools.persistence.js"
    );
    if (!shouldAutoStartSubscriptions()) {
      shouldSubscribe = false;
      logger.info("pools.refresh.phase.subscribe.manual_control", {
        message:
          "Pool persistence enabled - use retarget button to start subscriptions",
        cat: "pools",
      });
      try {
        emit("log", {
          level: "info",
          message:
            "pools:subscriptions require manual start - use retarget button",
          timestamp: new Date().toISOString(),
          context: { cat: "pools" },
        });
      } catch {}
  ...
  if (shouldSubscribe) {
    logger.info("pools.refresh.phase.subscribe", {
      enabled: shouldFetch,
      cat: "pools",
    });
    try {
      ...
      logger.warn("pools.refresh.phase.subscribe.failed", {
        error: String(e?.message || e),
        cat: "pools",
      });
    }
  ...
  const totalFinal = Object.values(finalCounts).reduce(
    (sum, v) => sum + v.amm + v.clmm + ((v as any).cpmm || 0),
    0
  );
  const totalFetched = phaseStats.fetch
    ? Object.values(phaseStats.fetch).reduce(
        (sum, v) => sum + v.amm + v.clmm + ((v as any).cpmm || 0),
        0
      )
    : 0;
  ...
  const refreshSummary = {
    ...
    dropRate:
      totalFetched > 0
        ? `${Math.round((1 - totalFinal / totalFetched) * 100)}%`
        : "n/a",
    phases: {
      ...
      universe: phaseStats.universe
        ? {
            ...
            dropped: Object.entries(phaseStats.universe.before).reduce(
              (sum, [k, v]) =>
                sum + (v - ((phaseStats.universe?.after as any)?.[k] || 0)),
              0
            ),
          }
        : undefined,
      minPools1: phaseStats.minPools1
        ? {
            ...
            before: Object.values(phaseStats.minPools1.before).reduce(
              (a, b) => a + b,
              0
            ),
            after: Object.values(phaseStats.minPools1.after).reduce(
              (a, b) => a + b,
              0
            ),
          }
      ...
      tvl: phaseStats.tvl
        ? {
            ...
            before: Object.values(phaseStats.tvl.before).reduce(
              (sum, v) => sum + v.a + v.c,
              0
            ),
            after: Object.values(phaseStats.tvl.after).reduce(
              (sum, v) => sum + v.a + v.c,
              0
            ),
          }
      ...
      activity: phaseStats.activity
        ? {
            maxInactiveHours: Math.round(
              (phaseStats.activity.maxInactiveMs || 0) / (60 * 60 * 1000)
            ),
      ...
      minPools2: phaseStats.minPools2
        ? {
            ...
            before: Object.values(phaseStats.minPools2.before).reduce(
              (a, b) => a + b,
              0
            ),
            after: Object.values(phaseStats.minPools2.after).reduce(
              (a, b) => a + b,
              0
            ),
          }
...
export async function getMeteoraBalancedPoolsCached(
  force = false,
  opts?: { skipUniverseFilter?: boolean; fetchV1?: boolean; fetchV2?: boolean }
): Promise<PoolsPayload> {
  const ttlMs = Number((CONFIG as any)?.meteoraBalanced?.cacheTtlMs || 300_000);
  const minForceGap = Math.max(
    1000,
    Number((CONFIG.system as any)?.poolRefreshMinGapMs || 3000)
  );
  (getMeteoraBalancedPoolsCached as any).__lastForceAt =
    (getMeteoraBalancedPoolsCached as any).__lastForceAt || 0;
  const now = Date.now();
  ...
  if (!force) {
    if (metbalCache.data && now - metbalCache.ts < ttlMs)
      return metbalCache.data;
    return metbalCache.data || { amm: [], clmm: [], cpmm: [] };
  }
  if (force) {
    const last = (getMeteoraBalancedPoolsCached as any).__lastForceAt as number;
    if (now - last < minForceGap && metbalCache.data)
      return metbalCache.data as any;
  ...
  metbalCache.inflight = (async () => {
    try {
      ...
      const union = await fetchMeteoraBalancedAllImpl({
        fetchV1,
        fetchV2,
      }).catch(async () => {
      ...
      try {
        const { saveRawNormalizedPools } = await import(
          "./pools.persistence.js"
        );
      ...
      metbalCache.ts = Date.now();
      poolsMetrics.meteora_balanced.fetches =
        (poolsMetrics.meteora_balanced.fetches || 0) + 1;
      poolsMetrics.meteora_balanced.lastMs = Date.now();
      poolsMetrics.meteora_balanced.lastAmm = (norm.amm || []).length;
      try {
        const d = diffNormalizedPools(
          prev || { amm: [], clmm: [], cpmm: [] },
          norm
        );
        emit("pools-update", {
        ...
        emit("pool-updates", {
          ...
          sample: { amm: d.amm.slice(0, 50), clmm: [] },
          ts: Date.now(),
        });
        ...
        if (!skipIncremental) {
          try {
            ...
            if (
              ...
              const thresh = Math.max(
                0,
                Number((CONFIG.system as any)?.graphDeltaRebuildThreshold || 0)
              );
              const delta =
                d.amm.length +
                d.clmm.length +
              ...
              if ((thresh === 0 && delta > 0) || delta >= thresh) {
                gmod.scheduleGraphRebuild(
                  undefined,
                  Math.max(
                    50,
                    Number(
                      (CONFIG.system as any)?.graphRebuildDebounceMs || 150
                    )
                  )
                );
              }
          ...
          try {
            logger.debug("graph.update.skipped_during_refresh", {
              source: "meteora_balanced",
...
export async function getPumpswapPoolsCached(
  force = false,
  opts?: { mints?: string[] }
): Promise<PoolsPayload> {
  ...
  (getPumpswapPoolsCached as any).__lastForceAt =
    (getPumpswapPoolsCached as any).__lastForceAt || 0;
  const now = Date.now();
  if (!force) {
    if (pumpswapCache.data && now - pumpswapCache.ts < ttlMs)
  ...
  if (force) {
    ...
    if (now - last < minForceGap && pumpswapCache.data)
      return pumpswapCache.data as any;
  ...
  pumpswapCache.inflight = (async () => {
    try {
      const mode = "graphql";
      try {
        logger.info("pumpswap.fetch start", {
          mode,
          ttlMs,
          shared: !!opts?.mints,
          cat: "pumpswap",
      ...
      try {
        emit("log", {
          level: "info",
          message: `arb:pools pumpswap.fetch start mode=${mode}`,
          timestamp: new Date().toISOString(),
          context: { cat: "arb" },
        });
      } catch {}
      ...
      poolsMetrics.pumpswap.enrichmentSuccess =
        enrichResult.metrics?.success || 0;
      ...
      try {
        const blist = new Set<string>(
          Array.isArray((CONFIG.system as any)?.tokenBlocklistMints)
            ? (CONFIG.system as any).tokenBlocklistMints
            : []
        );
        if (blist.size > 0) {
          ...
          if ((filtered.amm || []).length !== beforeAmm) {
            try {
              logger.info("pumpswap.blocklist.filter", {
                beforeAmm,
                afterAmm: filtered.amm.length,
              });
            } catch {}
          }
      ...
      try {
        ...
        const minAmmUsd = Math.max(
          globalAmm,
          Number((CONFIG as any)?.pumpswap?.minLiqBase || 0)
        );
        if (minAmmUsd > 0) {
          const beforeAmm = norm.amm.length;
          const amm = norm.amm.filter(
            (p) => Number((p as any).liquidity_base || 0) >= minAmmUsd
          );
          if (amm.length !== beforeAmm) {
            try {
              logger.info("pumpswap.filter tvl", {
                minAmmUsd,
                beforeAmm,
                afterAmm: amm.length,
                cat: "pumpswap",
      ...
      try {
        const enableValidation =
          (CONFIG as any)?.pumpswap?.validatePrices !== false;
        const maxSamples = Number(
          (CONFIG as any)?.pumpswap?.validationSamples || 5
        );
        if (enableValidation && norm.amm.length > 0) {
          ...
          const otherDexPools = new Map<
            string,
            Array<{ dex: string; price: number; liquidity: number; pool: any }>
          >();
          const addPool = (pools: any[], dexName: string) => {
            for (const p of pools) {
              ...
              otherDexPools.get(key)!.push({
                ...
                liquidity: Number(
                  (p as any).liquidity_base || (p as any).tvl_usd || 0
                ),
                pool: p,
              });
            }
          ...
          for (const pump of norm.amm) {
            ...
            if (deviation > 0.05) {
              ...
              if (samples.length < maxSamples) {
                samples.push({
                  pair: `${pump.mint_a.slice(0, 6)}.../${pump.mint_b.slice(
                    0,
                    6
                  )}...`,
        ...
        try {
          logger.warn("pumpswap.price.validation.error", {
            error: String(e?.message || e),
            cat: "pumpswap",
          });
        } catch {}
      ...
      try {
        emit("log", {
          level: "info",
          message: `arb:pools pumpswap.fetch ok amm=${norm.amm.length}`,
          timestamp: new Date().toISOString(),
          context: { cat: "arb" },
        });
      } catch {}
...
export async function getRaydiumPoolsNormalized(
  ...
  (getRaydiumPoolsNormalized as any).__lastForceAt =
    (getRaydiumPoolsNormalized as any).__lastForceAt || 0;
  const now = Date.now();
  ...
  if (!force) {
    if (raydiumCache.data && now - raydiumCache.ts < ttlMs)
      return raydiumCache.data;
  ...
  raydiumCache.inflight = (async () => {
    try {
      ...
      logger.info("raydium.fetch start", {
        ...
        includeAnchors:
          (CONFIG.system as any)?.includeAnchorsInUniverse !== false,
      ...
      try {
        emit("log", {
          level: "info",
          message: `arb:pools raydium.fetch start mode=${mode}`,
          timestamp: new Date().toISOString(),
          context: { cat: "arb" },
        });
      } catch {}
      ...
      try {
        ...
        for (const pool of norm.amm || []) {
          ...
          if (pool.market_program_id)
            staticData.market_program_id = pool.market_program_id;
          if (pool.market_bids) staticData.market_bids = pool.market_bids;
          if (pool.market_asks) staticData.market_asks = pool.market_asks;
          if (pool.market_event_queue)
            staticData.market_event_queue = pool.market_event_queue;
          if (pool.market_base_vault)
            staticData.market_base_vault = pool.market_base_vault;
          if (pool.market_quote_vault)
            staticData.market_quote_vault = pool.market_quote_vault;
          if (pool.market_authority)
            staticData.market_authority = pool.market_authority;
          ...
          if (
            staticData.programId ===
            "675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8"
          ) {
            staticData.amm_authority =
              (CONFIG as any)?.raydium?.ammV4Authority ||
              "5Q544fKrFoe6tsEbD7S8EmxGTJYAKtTVhAW5Q5pge4j1";
          } else {
            // For other versions, try to extract from pool data (fallback)
            if (pool.amm_authority)
              staticData.amm_authority = pool.amm_authority;
            if (pool.owner) staticData.owner = pool.owner; // Fallback field name
          }
          if (pool.amm_open_orders)
            staticData.amm_open_orders = pool.amm_open_orders;
          if (pool.amm_target_orders)
            staticData.amm_target_orders = pool.amm_target_orders;
            ...
            try {
              logger.info("raydium.amm.cache_authority_debug", {
                ...
                ctx: {
                  ...
                  hardcodedV4Authority:
                    (CONFIG as any)?.raydium?.ammV4Authority ||
                    "5Q544fKrFoe6tsEbD7S8EmxGTJYAKtTVhAW5Q5pge4j1",
                  allPoolFields: Object.keys(pool).filter(
                    (k) => k.includes("auth") || k.includes("owner")
                  ),
                },
              });
            } catch {}
        ...
        try {
          const targetPool = norm.amm?.find(
            (p: any) => p.id === "58oQChx4yWmvKdwLLZzBi4ChoCc2fqCUWBkwMihLYQo2"
          );
          if (targetPool) {
        ...
        logger.info("raydium.execution_cache.populated", {
          ...
          ctx: {
            ...
            clmmWithExBitmap: (norm.clmm || []).filter((p: any) => p.ex_bitmap)
              .length,
            clmmWithObservationState: (norm.clmm || []).filter(
              (p: any) => p.observation_state
            ).length,
          },
        });
      } catch (err) {
      ...
      try {
        if (!skipUniverseFilter) {
          ...
          const mode: any =
            (CONFIG.system as any)?.tokenUniverseMode || "jupiter";
          const uni = await computeTokenUniverse(mode);
          const beforeAmm = norm.amm.length,
            beforeClmm = norm.clmm.length;
          const scoped = filterPoolsByUniverse(
            norm as any,
            uni,
            !!(CONFIG.system as any)?.enableAnchorBridging
          );
          if (
            scoped.amm.length !== beforeAmm ||
            scoped.clmm.length !== beforeClmm
          ) {
            poolsMetrics.raydium.filteredAmm += beforeAmm - scoped.amm.length;
            poolsMetrics.raydium.filteredClmm +=
              beforeClmm - scoped.clmm.length;
            poolsMetrics.raydium.universe = String(mode);
            try {
              logger.info("raydium.universe.filter", {
                mode,
                beforeAmm,
                beforeClmm,
                afterAmm: scoped.amm.length,
                afterClmm: scoped.clmm.length,
              });
            } catch {}
          }
          norm = scoped as any;
      ...
      try {
        ...
        if (blist.size > 0) {
          ...
          const filtered = applyTokenMintBlocklist(norm as any, blist);
          if (
            filtered.amm.length !== beforeAmm ||
            filtered.clmm.length !== beforeClmm
          ) {
            try {
              logger.info("raydium.blocklist.filter", {
                beforeAmm,
                beforeClmm,
                afterAmm: filtered.amm.length,
                afterClmm: filtered.clmm.length,
              });
            } catch {}
          }
          norm = filtered as any;
      ...
      try {
        const globalAmm = Number((CONFIG.system as any)?.minAmmLiqBase ?? 0);
        const globalClmm = Number(
          (CONFIG.system as any)?.minClmmLiquidity ?? 0
        );
        const minAmmUsd = Math.max(
          globalAmm,
          Number((CONFIG.raydium as any)?.minAmmLiqBase || 0)
        );
        const minClmmUsd = Math.max(
          globalClmm,
          Number((CONFIG.raydium as any)?.minClmmLiquidity || 0)
        );
        if (minAmmUsd > 0 || minClmmUsd > 0) {
          ...
          const amm =
            minAmmUsd > 0
              ? norm.amm.filter(
                  (p) => Number((p as any).tvl_usd || 0) >= minAmmUsd
                )
              : norm.amm;
          const clmm =
            minClmmUsd > 0
              ? norm.clmm.filter(
                  (p) => Number((p as any).tvl_usd || 0) >= minClmmUsd
                )
              : norm.clmm;
          if (amm.length !== beforeAmm || clmm.length !== beforeClmm) {
            logger.info("raydium.filter tvl", {
              minAmmUsd,
              minClmmUsd,
              beforeAmm,
              beforeClmm,
              afterAmm: amm.length,
              afterClmm: clmm.length,
            });
          }
          norm = { amm, clmm } as any;
        }
      ...
      logger.info("raydium.fetch normalized", {
        amm: norm.amm.length,
      ...
      try {
        emit("log", {
          level: "info",
          message: `arb:pools raydium.fetch ok amm=${norm.amm.length} clmm=${norm.clmm.length}`,
          timestamp: new Date().toISOString(),
          context: { cat: "arb" },
        });
      } catch {}
      ...
      try {
        const { registerRaydiumPoolsForEligibility } = await import(
          "./pools.websockets.js"
        );
        const eligibilityStats = registerRaydiumPoolsForEligibility(
          norm.clmm || []
        );
        ...
        logger.debug("raydium.eligibility.register_failed", {
          cat: "pools",
          ctx: {
            error: String((eligibilityErr as any)?.message || eligibilityErr),
          },
        });
      }
      ...
      try {
        const { populateExecutionCacheFromPools } = await import(
          "./pools.persistence.js"
        );
      ...
      try {
        ...
        if (changed) {
          ...
          emit("log", {
            level: "info",
            message: `pools:update source=raydium amm=${nextAmm} clmm=${nextClmm}`,
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        }
      ...
      try {
        ...
        try {
          logger.info("pools.delta raydium", {
            updatedAmm: d.amm.length,
            updatedClmm: d.clmm.length,
            addedAmm: d.addedAmm,
            removedAmm: d.removedAmm,
        ...
        if (enable && !(refreshAllSources as any).__didWarmPricesOnce) {
          ...
          const collect = (pp: { amm: any[]; clmm: any[] }) => {
            ...
            return arr.map((p) => ({
              ...
              tvl: Number(
                (p as any)?.tvl_usd ?? (p as any)?.liquidity_display ?? 0
              ),
            }));
          };
          const items = [
          ...
          for (const it of ranked) {
            for (const m of it.mints || []) if (m) set.add(m);
          }
          ...
          if (ids.length) {
            const { fetchPricesByMints } = await import(
              "../jupiter/jupiter.js"
            );
            const fresh = await fetchPricesByMints(ids, {
              catOverride: "priceWarm",
              ignorePause: true,
            });
...
export async function getRaydiumPoolsGraphQL(
  force = false,
  opts?: { mints?: string[] }
): Promise<PoolsPayload> {
  try {
    ...
    logger.info("raydium.graphql.fetch.start", {
      mintCount: mints.length,
    ...
    const { fetchRaydiumGraphQL, normalizeRaydiumGraphQL } = await import(
      "./pools/raydiumGraphQL.js"
    );
    ...
    try {
      ...
      for (const pool of normalized.amm || []) {
        ...
        if ((pool as any).amm_open_orders || (pool as any).open_orders) {
          staticData.amm_open_orders =
            (pool as any).amm_open_orders || (pool as any).open_orders;
        }
        if ((pool as any).amm_target_orders || (pool as any).target_orders) {
          staticData.amm_target_orders =
            (pool as any).amm_target_orders || (pool as any).target_orders;
        }
        if (pool.lp_mint) staticData.lp_mint = pool.lp_mint;
      ...
      for (const pool of normalized.clmm || []) {
        ...
        if (pool.decimals_a != null) staticData.decimals_a = pool.decimals_a;
        if (pool.decimals_b != null) staticData.decimals_b = pool.decimals_b;
        if ((pool as any).observation_state)
          staticData.observation_state = (pool as any).observation_state;
        if ((pool as any).ex_bitmap)
          staticData.ex_bitmap = (pool as any).ex_bitmap;
        if (pool.account_a) staticData.account_a = pool.account_a;
        if (pool.account_b) staticData.account_b = pool.account_b;
        if (pool.tick_spacing) staticData.tick_spacing = pool.tick_spacing;
        ...
        const tickArrayLower =
          (pool as any).tick_array_lower ?? (pool as any).tickArrayLower;
        const tickArrayCenter =
          (pool as any).tick_array_center ?? (pool as any).tickArrayCenter;
        const tickArrayUpper =
          (pool as any).tick_array_upper ?? (pool as any).tickArrayUpper;

        if (tickArrayCenter && tickArrayCenter !== null) {
          hotData.tickArrays = {
            center: tickArrayCenter,
            lower:
              tickArrayLower && tickArrayLower !== null
                ? [tickArrayLower]
                : undefined,
            upper:
              tickArrayUpper && tickArrayUpper !== null
                ? [tickArrayUpper]
                : undefined,
          };
          hasHotData = true;
...
export async function getRaydiumClmmPoolsGraphQL(
  force = false,
  opts?: { mints?: string[] }
): Promise<PoolsPayload> {
  try {
    ...
    logger.info("raydium.clmm.graphql.fetch.start", {
      mintCount: mints.length,
      anchorCount: anchors.size,
      shared: !!opts?.mints,
      cat: "raydium-clmm",
    ...
    const { fetchRaydiumClmmGraphQL, normalizeRaydiumGraphQL } = await import(
      "./pools/raydiumGraphQL.js"
    );
    ...
    // Emit to websocket
    try {
      emit("pools:raydium-clmm", normalized);
    } catch {}
    ...
    try {
      ...
      for (const pool of normalized.clmm || []) {
        ...
        if (pool.account_b) staticData.account_b = pool.account_b;
        if (pool.tick_spacing) staticData.tick_spacing = pool.tick_spacing;
        if ((pool as any).sqrt_price_x64)
          staticData.sqrt_price_x64 = (pool as any).sqrt_price_x64;
        if ((pool as any).tick_current)
          staticData.tick_current = (pool as any).tick_current;
        if ((pool as any).liquidity)
          staticData.liquidity = (pool as any).liquidity;

        // Store native (pre-canonical) fields for execution
        if ((pool as any).native_mint_a)
          staticData.native_mint_a = (pool as any).native_mint_a;
        if ((pool as any).native_mint_b)
          staticData.native_mint_b = (pool as any).native_mint_b;
        if ((pool as any).native_account_a)
          staticData.native_account_a = (pool as any).native_account_a;
        if ((pool as any).native_account_b)
          staticData.native_account_b = (pool as any).native_account_b;
    ...
    logger.error("raydium.clmm.graphql.fetch.error", {
      error: String(err),
      cat: "raydium-clmm",
    });
...
export async function getRaydiumCpmmPoolsGraphQL(
  force = false,
  opts?: { mints?: string[] }
): Promise<PoolsPayload> {
  try {
    ...
    if ((CONFIG as any)?.raydiumCpmm?.enabled === false) {
      logger.info("raydium.cpmm.graphql.skipped", {
        reason: "disabled",
        cat: "raydium-cpmm",
      });
      return { amm: [], clmm: [], cpmm: [] };
    ...
    const { fetchRaydiumCpmmGraphQL, normalizeRaydiumCpmmGraphQL } =
      await import("./pools/raydiumCpmmGraphQL.js");
    ...
    const CACHE_PATH = joinPath(
      CONFIG.cacheDir,
      "raydium-cpmm-pools-graphql.json"
    );
    try {
      await writeJson(CACHE_PATH, normalized);
    } catch {}
    ...
    // Emit to websocket
    try {
      emit("pools:raydium-cpmm", normalized);
    } catch {}
    ...
    logger.error("raydium.cpmm.graphql.fetch.error", {
      error: String(err),
      cat: "raydium-cpmm",
    });
    return { amm: [], clmm: [], cpmm: [] };
...
export async function getOrcaPoolsCached(
  ...
  (getOrcaPoolsCached as any).__lastForceAt =
    (getOrcaPoolsCached as any).__lastForceAt || 0;
  const now = Date.now();
  // In non-forced mode, never initiate a fetch. Only return cached data (even if stale) or empty.
  if (!force) {
  ...
  if (force) {
    ...
    if (now - last < minForceGap && orcaCache.data)
      return orcaCache.data as any;
  ...
  orcaCache.inflight = (async () => {
    try {
      ...
      // This enables reactive filtering when tick moves in/out of safe range
      try {
        const { registerOrcaPoolsForEligibility } = await import(
          "./pools.websockets.js"
        );
        const eligibilityStats = registerOrcaPoolsForEligibility(
          data.clmm || []
        );
      ...
      try {
        ...
        if (changed) {
          ...
          emit("log", {
            level: "info",
            message: `pools:update source=orca amm=${nextAmm} clmm=${nextClmm}`,
            timestamp: new Date().toISOString(),
            context: { cat: "pools" },
          });
        }
...
export async function getOrcaPoolsNormalized(opts?: {
  skipUniverseFilter?: boolean;
}): Promise<PoolsPayload> {
  ...
  try {
    ...
    try {
      ...
      if (blist.size > 0) {
        ...
        if (
          ...
          try {
            logger.info("orca.blocklist.filter", {
              beforeAmm,
              beforeClmm,
              afterAmm: filtered.amm.length,
              afterClmm: filtered.clmm.length,
    ...
    try {
      const skipUniverseFilter = opts?.skipUniverseFilter === true;
      if (!skipUniverseFilter) {
        const uniModeAny: any =
          (CONFIG.system as any)?.tokenUniverseMode || "jupiter";
        const isTest =
          String((globalThis as any)?.process?.env?.NODE_ENV || "") === "test";
        const isVitest = !!(
          (globalThis as any)?.vi ||
          (globalThis as any)?.vitest ||
          String((globalThis as any)?.process?.env?.VITEST || "") === "true"
        );
        const skipUniverseMode =
          isTest || isVitest || String(uniModeAny) === "none";
        if (!skipUniverseMode) {
          const { computeTokenUniverse, filterPoolsByUniverse } = await import(
            "./universe.js"
          );
          ...
          if (
            ...
            try {
              logger.info("orca.universe.filter", {
                mode: uniModeAny,
                beforeAmm,
                beforeClmm,
                afterAmm: scoped.amm.length,
          ...
          try {
            logger.info("orca.universe.filter skip", {
              reason: isTest ? "test" : "mode:none",
            });
          } catch {}
        }
      }
    ...
    try {
      ...
      for (const pool of norm.clmm || []) {
        ...
        if ((pool as any).native_mint_b)
          staticData.native_mint_b = (pool as any).native_mint_b;
        if ((pool as any).was_swapped != null)
          staticData.was_swapped = (pool as any).was_swapped;
      ...
      try {
        logger.info("orca.execution_cache.populated", {
          ...
          ctx: {
            ...
            withVaults: (norm.clmm || []).filter(
              (p: any) => p.token_vault_a && p.token_vault_b
            ).length,
          },
        });
      } catch {}
    ...
    logger.info("orca.http normalized", {
      clmm: norm.clmm.length,
      canon: (CONFIG.system as any)?.canonicalizePairs || "none",
    });
    return norm;
...
export async function getOrcaPoolsGraphQL(
  force = false,
  opts?: { mints?: string[] }
): Promise<PoolsPayload> {
  try {
    ...
    const { fetchOrcaGraphQL, normalizeOrcaGraphQL } = await import(
      "./pools/orcaGraphQL.js"
    );
    ...
    try {
      ...
      for (const pool of normalized.clmm || []) {
        ...
        if ((pool as any).token_vault_a)
          staticData.token_vault_a = (pool as any).token_vault_a;
        if ((pool as any).token_vault_b)
          staticData.token_vault_b = (pool as any).token_vault_b;
        if (pool.account_a) staticData.account_a = pool.account_a;
        if (pool.account_b) staticData.account_b = pool.account_b;
      ...
      logger.info("orca.graphql.execution_cache.populated", {
        ...
        withVaults: (normalized.clmm || []).filter(
          (p: any) => p.token_vault_a && p.token_vault_b
        ).length,
        cat: "orca",
      });
    ...
    try {
      ...
      const orcaProgramId = new PublicKey(
        String(
      ...
      for (const pool of normalized.clmm || []) {
        try {
          ...
          const tickIndexNative =
            tickIndexCanonical !== undefined &&
            Number.isFinite(tickIndexCanonical)
              ? wasSwapped
                ? -tickIndexCanonical
                : tickIndexCanonical
              : undefined;
          ...
          if (tickIndexNative !== undefined && tickSpacing && tickSpacing > 0) {
            try {
              ...
              const realIndex = Math.floor(
                Number(tickIndexNative) / ticksInArray
              );
          ...
          if (
            ...
            executionCache.setHot(pool.id, {
              ...
              ...(tickIndexNative !== undefined
                ? { currentTickIndex: tickIndexNative }
                : {}),
...
export async function getMeteoraPoolsCached(
  ...
  (getMeteoraPoolsCached as any).__lastForceAt =
    (getMeteoraPoolsCached as any).__lastForceAt || 0;
  const now = Date.now();
  // Capture opts for use in closure
  const skipUniverseFilter = opts?.skipUniverseFilter === true;
  if (!force) {
    if (meteoraCache.data && now - meteoraCache.ts < ttlMs)
  ...
  if (force) {
    ...
    if (now - last < minForceGap && meteoraCache.data)
      return meteoraCache.data as any;
  ...
  meteoraCache.inflight = (async () => {
    try {
      const mode = "http";
      try {
        logger.info("meteora.fetch start", {
          mode,
          ttlMs,
          uniMode: (CONFIG.system as any)?.tokenUniverseMode || "jupiter",
          anchorBridging: !!(CONFIG.system as any)?.enableAnchorBridging,
      ...
      try {
        emit("log", {
          level: "info",
          message: `arb:pools meteora.fetch start mode=${mode}`,
          timestamp: new Date().toISOString(),
          context: { cat: "arb" },
        });
      } catch {}
      ...
      try {
        ...
        if (blist.size > 0) {
          ...
          if ((filtered.clmm || []).length !== beforeClmm) {
            try {
              logger.info("meteora.blocklist.filter", {
                beforeClmm,
                afterClmm: filtered.clmm.length,
              });
            } catch {}
      ...
      try {
        if (!skipUniverseFilter) {
          ...
          if (prefilter) {
            const { computeTokenUniverse, filterPoolsByUniverse } =
              await import("./universe.js");
            const mode: any =
              (CONFIG.system as any)?.tokenUniverseMode || "jupiter";
            const uni = await computeTokenUniverse(mode);
            const beforeClmm = norm.clmm.length;
            ...
            if (scoped.clmm.length !== beforeClmm) {
              try {
                logger.info("meteora.universe.filter", {
                  mode,
                  beforeClmm,
                  afterClmm: scoped.clmm.length,
                });
      ...
      try {
        ...
        for (const pool of norm.clmm || []) {
          ...
          if (pool.decimals_a != null) staticData.decimals_a = pool.decimals_a;
          if (pool.decimals_b != null) staticData.decimals_b = pool.decimals_b;
          if (pool.token_program_a)
            staticData.token_program_a = pool.token_program_a;
          if (pool.token_program_b)
            staticData.token_program_b = pool.token_program_b;

          // Store execution-critical accounts (bin_array_bitmap_extension)
          if (pool.bin_array_bitmap_extension) {
            staticData.bin_array_bitmap_extension =
              pool.bin_array_bitmap_extension;
          }
          if ((pool as any).bin_array_lower)
            staticData.bin_array_lower = (pool as any).bin_array_lower;
          if ((pool as any).bin_array_upper)
            staticData.bin_array_upper = (pool as any).bin_array_upper;
          ...
          if ((pool as any).bin_step)
            staticData.binStep = (pool as any).bin_step;
        ...
        try {
          logger.info("meteora.execution_cache.populated", {
            ...
            ctx: {
              poolCount: (norm.clmm || []).length,
              withBitmapExt: (norm.clmm || []).filter(
                (p: any) => p.bin_array_bitmap_extension
              ).length,
            },
          });
        } catch {}
        ...
        try {
          const { registerPoolsForBitmapWatch } = await import(
            "./pools.websockets.js"
          );
      ...
      try {
        logger.info("meteora.fetch normalized", {
          clmm: norm.clmm.length,
          ms: Date.now() - t0,
          cat: "meteora",
        });
      ...
      try {
        emit("log", {
          level: "info",
          message: `arb:pools meteora.fetch ok clmm=${norm.clmm.length}`,
          timestamp: new Date().toISOString(),
          context: { cat: "arb" },
        });
      } catch {}
...
export async function getMeteoraPoolsGraphQL(
  force = false,
  opts?: { mints?: string[] }
): Promise<PoolsPayload> {
  try {
    ...
    const { fetchMeteoraGraphQL, normalizeMeteoraGraphQL } = await import(
      "./pools/meteoraGraphQL.js"
    );
    ...
    try {
      ...
      for (const pool of normalized.clmm || []) {
        ...
        if (pool.decimals_a != null) staticData.decimals_a = pool.decimals_a;
        if (pool.decimals_b != null) staticData.decimals_b = pool.decimals_b;
        if ((pool as any).token_program_a)
          staticData.token_program_a = (pool as any).token_program_a;
        if ((pool as any).token_program_b)
          staticData.token_program_b = (pool as any).token_program_b;
        ...
        if (
          (binArrayLower && binArrayLower !== null) ||
          (binArrayUpper && binArrayUpper !== null)
        ) {
          hotData.binArrays = {
            lower:
              binArrayLower && binArrayLower !== null
      ...
      logger.info("meteora.graphql.execution_cache.populated", {
        clmm: normalized.clmm.length,
        withBitmapExt: (normalized.clmm || []).filter(
          (p: any) => p.bin_array_bitmap_extension
        ).length,
        withTokenPrograms: (normalized.clmm || []).filter(
          (p: any) => p.token_program_a && p.token_program_b
        ).length,
        cat: "meteora",
      });
    } catch (e: any) {
