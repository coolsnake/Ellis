...
export const CONFIG = {
  ...
  drift: {
    ...
    liquidator: {
      ...
      maxUsersPerPriceTick: Number(process.env.DRIFT_LIQ_MAX_USERS_PER_PRICE_TICK || 40),
      targetCooldownMs: Number(process.env.DRIFT_LIQ_TARGET_COOLDOWN_MS || 7000),
      statsIntervalMs: Number(process.env.DRIFT_LIQ_STATS_INTERVAL_MS || 15000),
      usersListLimit: Number(process.env.DRIFT_LIQ_USERS_LIST_LIMIT || 200),
      recoveryBuffer: Number(process.env.DRIFT_LIQ_RECOVERY_BUFFER || 0.05),
      ...
      discoveryIntervalMs: Number(process.env.DRIFT_LIQ_DISCOVERY_INTERVAL_MS || 30000),
      maxNewUsersPerTick: Number(process.env.DRIFT_LIQ_MAX_NEW_USERS_PER_TICK || 250),
      scanBatchSize: Number(process.env.DRIFT_LIQ_SCAN_BATCH_SIZE || 200),
      // Helius GPA bootstrap enumeration (optional)
      enumerateAllOnStart: (process.env.DRIFT_LIQ_ENUMERATE_ON_START || 'false') === 'true',
