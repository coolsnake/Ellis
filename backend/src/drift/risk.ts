...
export function canPlaceOrders(
  ...
  if (cfg.fundingGuard) {
    ...
    if (maxApy > 0 && typeof fundingApy === 'number' && isFinite(fundingApy)) {
      if (Math.abs(fundingApy) > maxApy) {
        return { ok: false, reason: `funding apy ${fundingApy.toFixed(4)} exceeds max ${maxApy}` };
      }
    }
  }
  return { ok: true };
