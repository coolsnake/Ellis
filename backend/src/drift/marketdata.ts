...
export async function fetchDlobTopMakers(marketIndex: number): Promise<DlobTopMakers | null> {
  const base = (CONFIG as any).drift?.dlobUrl || 'https://dlob.drift.trade';
  const cluster = (CONFIG as any).drift?.cluster || 'mainnet-beta';
  const url = `${base}/topMakers?marketIndex=${marketIndex}&marketType=perp&cluster=${encodeURIComponent(cluster)}`;
  const fetchAny: any = (globalThis as any).fetch;
  try {
    ...
    logger.warn('drift.dlob.fetch_topMakers_failed', { error: String(e?.message || e), url, marketIndex, cat: 'drift' });
    return null;
  }
}
...
export async function fetchDlobL3Makers(marketIndex: number): Promise<string[]> {
  const base = (CONFIG as any).drift?.dlobUrl || 'https://dlob.drift.trade';
  const cluster = (CONFIG as any).drift?.cluster || 'mainnet-beta';
  const url = `${base}/l3?marketIndex=${marketIndex}&marketType=perp&cluster=${encodeURIComponent(cluster)}`;
  const fetchAny: any = (globalThis as any).fetch;
  ...
  try {
    ...
    logger.warn('drift.dlob.fetch_l3_failed', { error: String(e?.message || e), url, marketIndex, cat: 'drift' });
    return [];
  }
}
