...
fn reciprocity_sol_usdc() {
    ...
    let fee_b = 10i64;
    let liq = 1.0;
    g.upsert_edge("Raydium", &a, &b, EdgeData{ rate_effective: (1.0/200.0)*(1.0- (fee_a as f64)/10_000.0), fee_bps: fee_a, liquidity: liq, dex: "Raydium".into(), pool_id: "pid".into(), liquidity_display: liq });
    g.upsert_edge("Raydium", &b, &a, EdgeData{ rate_effective: 200.0*(1.0- (fee_b as f64)/10_000.0), fee_bps: fee_b, liquidity: liq, dex: "Raydium".into(), pool_id: "pid-rev".into(), liquidity_display: liq });
    // Product should match (1-fees)
    let mut fwd = 0.0; let mut rev = 0.0;
    for e in g.g.edge_references() { let w = e.weight(); if w.pool_id=="pid" { fwd = w.rate_effective; } if w.pool_id=="pid-rev" { rev = w.rate_effective; } }
