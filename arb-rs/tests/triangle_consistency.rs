...
fn triangle_consistency() {
    ...
    g.upsert_edge(
        ...
        &sol,
        EdgeData {
            rate_effective: 200.0 * (1.0 - (f1 as f64) / 10_000.0),
            fee_bps: f1,
            liquidity: liq,
            dex: "Raydium".into(),
            pool_id: "t1".into(),
    ...
    g.upsert_edge(
        ...
        &x,
        EdgeData {
            rate_effective: 2.0 * (1.0 - (f2 as f64) / 10_000.0),
            fee_bps: f2,
            liquidity: liq,
            dex: "Raydium".into(),
            pool_id: "t2".into(),
    ...
    g.upsert_edge(
        ...
        &usdc,
        EdgeData {
            rate_effective: 0.0025 * (1.0 - (f3 as f64) / 10_000.0),
            fee_bps: f3,
            liquidity: liq,
            dex: "Raydium".into(),
            pool_id: "t3".into(),
    ...
    for (u, v) in vec![(&usdc, &sol), (&sol, &x), (&x, &usdc)] {
        ...
        for e in g.g.edges_connecting(ui, vi) {
            prod *= e.weight().rate_effective;
        }
    }
    let exp = (1.0 - (f1 as f64) / 10_000.0)
        * (1.0 - (f2 as f64) / 10_000.0)
        * (1.0 - (f3 as f64) / 10_000.0);
    assert!(
        prod / exp > 0.98 && prod / exp < 1.02,
        "prod={}, exp={}",
        prod,
        exp
    );
